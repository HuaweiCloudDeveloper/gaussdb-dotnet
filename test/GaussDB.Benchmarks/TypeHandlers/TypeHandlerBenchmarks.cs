...
public abstract class TypeHandlerBenchmarks<T>
{
    ...
    protected TypeHandlerBenchmarks(PgConverter handler)
    {
        ...
        _readBuffer = new GaussDBReadBuffer(null, stream, null, GaussDBReadBuffer.MinimumSize, GaussDBWriteBuffer.UTF8Encoding, GaussDBWriteBuffer.RelaxedUTF8Encoding);
        _writeBuffer =  new GaussDBWriteBuffer(null, stream, null, GaussDBWriteBuffer.MinimumSize, GaussDBWriteBuffer.UTF8Encoding);
        _reader = new PgReader(_readBuffer);
        _writer = new PgWriter(new GaussDBBufferWriter(_writeBuffer));
