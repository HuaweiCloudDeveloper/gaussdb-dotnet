...
/// </summary>
[TestFixture(GaussDBDbType.Jsonb)]
[TestFixture(GaussDBDbType.Json)]
public class JsonNetTests(GaussDBDbType gaussdbDbType) : TestBase
{
    [Test]
    ...
    public async Task Deserialize_failure()
    {
        ...
        await using var cmd = new GaussDBCommand($@"SELECT '[1, 2, 3]'::{_pgTypeName}", conn);
    ...
    public async Task Bug3464()
    {
        ...
        cmd.Parameters.AddWithValue("p1", expected).GaussDBDbType = gaussdbDbType;
        cmd.Parameters.AddWithValue("p2", expected).GaussDBDbType = gaussdbDbType;
    ...
    [Test]
    public async Task Write_jobject_without_gaussdbdbtype()
    {
        await using var conn = await JsonDataSource.OpenConnectionAsync();
    ...
    public async Task Write_jobject_array_without_gaussdbdbtype()
    {
        await using var conn = await JsonDataSource.OpenConnectionAsync();
        var tableName = await TestUtil.CreateTempTable(conn, "key SERIAL PRIMARY KEY, ingredients json[]");
    ...
    readonly string _pgTypeName = gaussdbDbType.ToString().ToLower();
    ...
    GaussDBDataSource JsonDataSource = default!;
}
*/
