...
public class NetTopologySuiteTests : TestBase
{
    ...
    public async Task Array()
    {
        ...
        await AssertType(
            ...
            '{' + GetSqlLiteral(point) + '}',
            "geometry[]",
            GaussDBDbType.Geometry | GaussDBDbType.Array,
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new GaussDBCommand("SELECT st_makepoint(1,1)", conn);
    ...
    public async Task Roundtrip_geometry_geography()
    {
        ...
        await using (var cmd = new GaussDBCommand("INSERT INTO data (geom, geog) VALUES (@p, @p)", conn))
        {
            cmd.Parameters.AddWithValue("@p", point);
            cmd.ExecuteNonQuery();
        }

        await using (var cmd = new GaussDBCommand("SELECT geom, geog FROM data", conn))
        await using (var reader = cmd.ExecuteReader())
        {
            reader.Read();
    ...
    public async Task Concurrency_test()
    {
        ...
        await Task.WhenAll(Enumerable.Range(0, 30).Select(i => Task.Run(async () =>
        {
            for (var i = 0; i < 1000; i++)
            {
                ...
                await using (var cmd = new GaussDBCommand($"SELECT * FROM {table}", connection))
    ...
    protected ValueTask<GaussDBConnection> OpenConnectionAsync(string? connectionString = null, Ordinates handleOrdinates = Ordinates.None)
    {
    ...
    protected override GaussDBDataSource DataSource => _xyDataSource ?? throw new InvalidOperationException();
    GaussDBDataSource? _xyDataSource;

    ConcurrentDictionary<Ordinates, GaussDBDataSource> NtsDataSources = new();
}
*/
