...
public class ReaderTests : MultiplexingTestBase
{
    ...
    public async Task Resumable_non_consumed_to_non_resumable()
    {
        ...
        await using var cmd = new GaussDBCommand( "SELECT 'aaaaaaaa', 1", conn);
    ...
    public async Task Seek_columns()
    {
        ...
        using var cmd = new GaussDBCommand("SELECT 1,2,3", conn);
    ...
    public async Task No_resultset()
    {
        ...
        using (var cmd = new GaussDBCommand($"INSERT INTO {table} VALUES (8)", conn))
        using (var reader = await cmd.ExecuteReaderAsync(Behavior))
        {
        ...
        using (var cmd = new GaussDBCommand($"SELECT 1; INSERT INTO {table} VALUES (8)", conn))
        using (var reader = await cmd.ExecuteReaderAsync(Behavior))
        {
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand("SELECT 1 AS foo WHERE FALSE", conn);
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
        Assert.That(reader.Read(), Is.False);
    ...
    public async Task FieldCount()
    {
        ...
        using var cmd = new GaussDBCommand("SELECT 1; SELECT 2,3", conn);
        using (var reader = await cmd.ExecuteReaderAsync(Behavior))
        {
            Assert.That(reader.FieldCount, Is.EqualTo(1));
            Assert.That(reader.Read(), Is.True);
    ...
    public async Task RecordsAffected()
    {
        ...
        {
            cmd = new GaussDBCommand($"MERGE INTO {table} S USING (SELECT 2 as int) T ON T.int = S.int WHEN MATCHED THEN UPDATE SET int = S.int", conn);
            reader = await cmd.ExecuteReaderAsync(Behavior);
            reader.Close();
            Assert.That(reader.RecordsAffected, Is.EqualTo(1));
        }
...
UPDATE {table} SET name='b' WHERE name='doesnt_exist';";
        ...
        using (var cmd = new GaussDBCommand($"SELECT name FROM {table}; DELETE FROM {table}", conn))
        {
            using var reader = await cmd.ExecuteReaderAsync(Behavior);
...
#pragma warning restore CS0618
    ...
    public async Task Get_string_with_parameter()
    {
        ...
        var command = new GaussDBCommand($"SELECT name FROM {table} WHERE name = :value;", conn);
        ...
        //param.GaussDBDbType = GaussDBDbType.Text;
...
INSERT INTO {table} (name) VALUES ('Text with '' single quote');");
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var command = new GaussDBCommand(@"SELECT 'Random text' AS real_column", conn);
        using var dr = await command.ExecuteReaderAsync(Behavior);
        dr.Read();
    ...
    {
        using var conn = await OpenConnectionAsync();
        using (var cmd = new GaussDBCommand(@"SELECT 1::INT4 AS some_column", conn))
        using (var reader = await cmd.ExecuteReaderAsync(Behavior))
        {
            reader.Read();
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new GaussDBCommand(@"SELECT 1::INT4 AS some_column", conn);
    ...
    public async Task GetPostgresType()
    {
        ...
        using (var cmd = new GaussDBCommand(@"SELECT 1::INTEGER AS some_column", conn))
        using (var reader = await cmd.ExecuteReaderAsync(Behavior))
        {
            reader.Read();
    ...
    public async Task GetDataTypeName(string typeName, string? normalizedName = null)
    {
        ...
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand($"SELECT NULL::{typeName} AS some_column", conn);
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
        reader.Read();
    ...
    public async Task GetDataTypeName_enum()
    {
        ...
        conn.ReloadTypes();
        await using var cmd = new GaussDBCommand($"SELECT 'one'::{typeName}", conn);
        await using var reader = await cmd.ExecuteReaderAsync(Behavior);
        await reader.ReadAsync();
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var command = new GaussDBCommand(@"SELECT 1 AS some_column", conn);
        using var dr = await command.ExecuteReaderAsync(Behavior);
        dr.Read();
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand("SELECT 'foo'::TEXT", conn);
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
        reader.Read();
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var command = new GaussDBCommand(@"SELECT 'hello', 1, '2014-01-01'::DATE", conn);
        using (var dr = await command.ExecuteReaderAsync(Behavior))
        {
            dr.Read();
    ...
    public async Task ExecuteReader_getting_empty_resultset_with_output_parameter()
    {
        using var conn = await OpenConnectionAsync();
        var table = await CreateTempTable(conn, "name TEXT");
        var command = new GaussDBCommand($"SELECT * FROM {table} WHERE name = NULL;", conn);
        var param = new GaussDBParameter("some_param", GaussDBDbType.Varchar);
    ...
    public async Task Get_value_from_empty_resultset()
    {
        using var conn = await OpenConnectionAsync();
        var table = await CreateTempTable(conn, "name TEXT");
        using var command = new GaussDBCommand($"SELECT * FROM {table} WHERE name = :value;", conn);
    ...
    {
        using var conn = await OpenConnectionAsync();
        var command = new GaussDBCommand("SELECT 1", conn);
        using var dr = await command.ExecuteReaderAsync(Behavior);
        while (dr.Read()) {}
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var command = new GaussDBCommand(@"SELECT 1; SELECT 2", conn);
...
LANGUAGE 'plpgsql';
        ...
        using var cmd = new GaussDBCommand($"SELECT {function}()", conn);
...
LANGUAGE 'plpgsql'");
        ...
        // We use GaussDBConnection.CreateCommand to test that the command isn't recycled when referenced in an exception
...
LANGUAGE 'plpgsql'");
    ...
    public async Task SchemaOnly_next_result_beyond_end()
    {
        ...
        using var cmd = new GaussDBCommand($"SELECT * FROM {table}", conn);
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand(@"select aggfnoid from pg_aggregate", conn);
        using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var command = new GaussDBCommand(@"SELECT 0, 1 AS some_column WHERE 1=0", conn);
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var command = new GaussDBCommand("select 123 as FIELD1", conn);
        using var reader = await command.ExecuteReaderAsync(Behavior);
        reader.Read();
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var command = new GaussDBCommand("select 123 as ｦｧｨｩｪｫｬ", conn);
        using var reader = await command.ExecuteReaderAsync(Behavior);
        reader.Read();
    ...
    public async Task Reader_is_still_open()
    {
        ...
        Assert.That(() => conn.ExecuteNonQuery("SELECT 1"), Throws.Exception.TypeOf<GaussDBOperationInProgressException>());
        Assert.That(async () => await conn.ExecuteScalarAsync("SELECT 1"), Throws.Exception.TypeOf<GaussDBOperationInProgressException>());

        using var cmd2 = new GaussDBCommand("SELECT 2", conn);
        Assert.That(() => cmd2.ExecuteReader(Behavior), Throws.Exception.TypeOf<GaussDBOperationInProgressException>());
        if (!IsMultiplexing)
            Assert.That(() => cmd2.Prepare(), Throws.Exception.TypeOf<GaussDBOperationInProgressException>());
    }
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand("SELECT @p1, @p2::TEXT", conn);
        cmd.Parameters.Add(new GaussDBParameter("p1", DbType.String) { Value = DBNull.Value });
        cmd.Parameters.Add(new GaussDBParameter { ParameterName = "p2", Value = DBNull.Value });
    ...
    public async Task HasRows_without_resultset()
    {
        using var conn = await OpenConnectionAsync();
        var table = await CreateTempTable(conn, "name TEXT");
        using var command = new GaussDBCommand($"DELETE FROM {table} WHERE name = 'unknown'", conn);
        using var reader = await command.ExecuteReaderAsync(Behavior);
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var command = new GaussDBCommand("SELECT CAST('1 hour' AS interval) AS dauer", conn);
        using var dr = await command.ExecuteReaderAsync(Behavior);
    ...
    public async Task GetFieldValueAsync_AsyncRead()
    {
        ...
        using var cmd = new GaussDBCommand("irrelevant", conn);
        var reader = await cmd.ExecuteReaderAsync(Behavior);
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand("SELECT 1, 2", conn);
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
        reader.Read();
    }
...
LANGUAGE plpgsql VOLATILE";
    ...
    public async Task Invalid_cast()
    {
        ...
        using (var cmd = new GaussDBCommand("SELECT 'foo'", conn))
        using (var reader = await cmd.ExecuteReaderAsync(Behavior))
        {
            reader.Read();
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand($"SELECT generate_series(1, {conn.Settings.ReadBufferSize})", conn);
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
    ...
    public async Task Nullable_scalar()
    {
        ...
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand("SELECT @p1, @p2", conn);
        var p1 = new GaussDBParameter { ParameterName = "p1", Value = DBNull.Value, GaussDBDbType = GaussDBDbType.Smallint };
        var p2 = new GaussDBParameter { ParameterName = "p2", Value = (short)8 };
        Assert.That(p2.GaussDBDbType, Is.EqualTo(GaussDBDbType.Smallint));
    ...
    public async Task Bug2913_reading_previous_query_messages()
    {
        ...
        var secondQuery = Task.Run(async () =>
        {
            ...
            using var secondCmd = new GaussDBCommand(@"SELECT 1; SELECT 2;", secondConn);
        ...
        {
            // Executing a query, which fails with GaussDBException on reader disposing, as NotExistingTable doesn't exist
            using var firstCmd = new GaussDBCommand(@"SELECT 1; SELECT * FROM NotExistingTable;", firstConn);
        ...
        using var thirdCmd = new GaussDBCommand(@"SELECT 1; SELECT 2;", thirdConn);
    ...
    public async Task Bug3772()
    {
        ...
        using var cmd = new GaussDBCommand("SELECT some_int, some_byte FROM some_table", conn);
        await using var reader = await cmd.ExecuteReaderAsync(Behavior);
    ...
    public async Task Dispose_does_not_swallow_exceptions([Values(true, false)] bool async)
    {
        ...
        if (async)
            Assert.Throws<GaussDBException>(() => reader.Dispose());
        else
            Assert.ThrowsAsync<GaussDBException>(async () => await reader.DisposeAsync());
    }
    ...
    public async Task GetStream_second_time_throws([Values(true, false)] bool isAsync)
    {
        ...
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand($"SELECT {EncodeByteaHex(expected)}::bytea", conn);
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
        ...
        [ValueSource(nameof(GetStreamCases))] (T Generic, byte[] Binary) value)
    {
        ...
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand("SELECT @p, @p", conn) { Parameters = { new GaussDBParameter("p", value.Generic) } };
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
    ...
    public async Task Open_stream_when_changing_columns([Values(true, false)] bool isAsync)
    {
        ...
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand(@"SELECT @p, @p", conn);
        var data = new byte[] { 1, 2, 3 };
        cmd.Parameters.Add(new GaussDBParameter("p", data));
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
        reader.Read();
    ...
    public async Task GetBytes_with_null([Values(true, false)] bool isAsync)
    {
        ...
        using var cmd = new GaussDBCommand($"SELECT bytes FROM {table}", conn);
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
        reader.Read();
    ...
    static Func<GaussDBDataReader, int, Task<Stream>> BuildStreamGetter(bool isAsync)
        => isAsync
    ...
    public async Task GetStream_after_consuming_column_throws([Values] bool async)
    {
        ...
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new GaussDBCommand(@"SELECT '\xDEADBEEF'::bytea", conn);
    ...
    public async Task GetChars()
    {
        ...
        using var cmd = new GaussDBCommand(queryText, conn);
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
        reader.Read();
    ...
    public async Task GetChars_AdvanceConsumed()
    {
        ...
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand($"SELECT '{value}'", conn);
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
        reader.Read();
    ...
    {
        Func<GaussDBDataReader, int, Task<TextReader>> textReaderGetter;
        if (isAsync)
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand(@"SELECT 'some_text', 'some_text'", conn);
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
        reader.Read();
    ...
    public async Task GetChars_when_null()
    {
        var buf = new char[8];
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand("SELECT NULL::TEXT", conn);
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
        reader.Read();
        Assert.That(reader.IsDBNull(0), Is.True);
    ...
    public async Task Reader_is_reused()
    {
        ...
        using var conn = await OpenConnectionAsync();
        GaussDBDataReader reader1;

        using (var cmd = new GaussDBCommand("SELECT 8", conn))
        ...
        using (var cmd = new GaussDBCommand("SELECT 9", conn))
    ...
    public async Task GetTextReader_after_consuming_column_throws([Values] bool async)
    {
        ...
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new GaussDBCommand("SELECT 'foo'", conn);
        await using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SequentialAccess);
        await reader.ReadAsync();
    ...
    [Test, Description("Tests that everything goes well when a type handler generates a GaussDBSafeReadException")]
    public async Task SafeReadException()
    {
        ...
        await using var cmd = new GaussDBCommand(@"SELECT 1, 'hello'", connection);
        await using var reader = await cmd.ExecuteReaderAsync(Behavior);
        await reader.ReadAsync();
    ...
    [Test, Description("Tests that when a type handler generates an exception that isn't a GaussDBSafeReadException, the connection is properly broken")]
    ...
    [Test, Description("Cancels ReadAsync via the GaussDBCommand.Cancel, with successful PG cancellation")]
    public async Task ReadAsync_cancel_command_soft()
    {
        ...
        using var cmd = new GaussDBCommand("SELECT some_int FROM some_table", conn);
        await using (var reader = await cmd.ExecuteReaderAsync(Behavior))
        {
    ...
    public async Task GetFieldValueAsync_sequential_cancel([Values(true, false)] bool passCancelledToken)
    {
        ...
        using var cmd = new GaussDBCommand("SELECT some_bytea FROM some_table", conn);
        await using var reader = await cmd.ExecuteReaderAsync(Behavior);
    ...
    public async Task IsDBNullAsync_sequential_cancel([Values(true, false)] bool passCancelledToken)
    {
        ...
        using var cmd = new GaussDBCommand("SELECT some_bytea, some_int FROM some_table", conn);
        await using var reader = await cmd.ExecuteReaderAsync(Behavior);
    ...
    public async Task Cancel_multiplexing_disabled()
    {
        ...
        await using var dataSource = CreateDataSource();
        await using var conn = await dataSource.OpenConnectionAsync();
        await using var cmd = new GaussDBCommand("SELECT generate_series(1, 100); SELECT generate_series(1, 100)", conn);
        await using var reader = await cmd.ExecuteReaderAsync(Behavior);
    ...
    public async Task GetFieldValueAsync_sequential_timeout()
    {
        ...
        var exception = Assert.ThrowsAsync<GaussDBException>(async () => await task)!;
        Assert.That(exception.InnerException, Is.TypeOf<TimeoutException>());
