...
public class DataSourceTests : TestBase
{
    ...
    {
        await using var dataSource = GaussDBDataSource.Create(ConnectionString);
    ...
    public void Clear()
    {
        using var dataSource = GaussDBDataSource.Create(ConnectionString);
    ...
    {
        if (string.IsNullOrEmpty(new GaussDBConnectionStringBuilder(ConnectionString).Password))
            Assert.Fail("No password in default connection string, test cannot run");

        using var dataSource = GaussDBDataSource.Create(ConnectionString);
        var parsedConnectionString = new GaussDBConnectionStringBuilder(dataSource.ConnectionString);
    ...
    {
        var dataSource = GaussDBDataSource.Create(ConnectionString);
    ...
    {
        var connectionStringBuilder = new GaussDBConnectionStringBuilder(ConnectionString) { Pooling = false };
        var dataSource = GaussDBDataSource.Create(connectionStringBuilder);
    ...
    {
        await using DbDataSource dataSource = GaussDBDataSource.Create(ConnectionString);
        await using var connection = async
            ? await dataSource.OpenConnectionAsync()
            : dataSource.OpenConnection();
        Assert.That(connection.State, Is.EqualTo(ConnectionState.Open));
    ...
    public async Task Executing_command_on_disposed_datasource([Values] bool multiplexing)
    {
        ...
        DbDataSource dataSource = GaussDBDataSource.Create(csb.ConnectionString);
    ...
    public async Task Multiplexing_connectionless_command_open_connection()
    {
        ...
        await using var dataSource = GaussDBDataSource.Create(csb.ConnectionString);
