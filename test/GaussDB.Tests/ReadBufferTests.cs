...
class ReadBufferTests
{
    ...
    {
        Writer
            .Write(GaussDBWriteBuffer.UTF8Encoding.GetBytes(new string("foo")))
            .WriteByte(0)
            .Write(GaussDBWriteBuffer.UTF8Encoding.GetBytes(new string("bar")))
            .WriteByte(0);
    ...
    {
        Writer.Write(GaussDBWriteBuffer.UTF8Encoding.GetBytes(new string("Chunked ")));
        await ReadBuffer.Ensure(1, async: true);
        var task = ReadBuffer.ReadNullTerminatedString(async: true);
        Assert.That(!task.IsCompleted);

        Writer
            .Write(GaussDBWriteBuffer.UTF8Encoding.GetBytes(new string("string")))
            .WriteByte(0)
            .Write(GaussDBWriteBuffer.UTF8Encoding.GetBytes(new string("bar")))
            .WriteByte(0);
        Assert.That(task.IsCompleted);
    ...
    public void SetUp()
    {
        ...
        ReadBuffer = new GaussDBReadBuffer(null, stream, null, GaussDBReadBuffer.DefaultSize, GaussDBWriteBuffer.UTF8Encoding, GaussDBWriteBuffer.RelaxedUTF8Encoding);
        Writer = stream.Writer;
    }

    // ReSharper disable once InconsistentNaming
    GaussDBReadBuffer ReadBuffer = default!;
    // ReSharper disable once InconsistentNaming
