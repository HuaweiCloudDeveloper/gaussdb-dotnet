...
public class TransactionTests(MultiplexingMode multiplexingMode) : MultiplexingTestBase(multiplexingMode)
{
    ...
    public async Task Commit([Values(PrepareOrNot.NotPrepared, PrepareOrNot.Prepared)] PrepareOrNot prepare)
    {
        ...
        await using (tx)
        {
            var cmd = new GaussDBCommand($"INSERT INTO {table} (name) VALUES ('X')", conn, tx);
        ...
        // With multiplexing we can't assume that disposed GaussDBTransaction will throw ObjectDisposedException
        // Because disposed GaussDBTransaction might be reused by another thread
        if (!IsMultiplexing)
    ...
    public async Task Rollback_failed_transaction_with_timeout()
    {
        ...
        var tx = conn.BeginTransaction();
        using var cmd = new GaussDBCommand("BAD QUERY", conn, tx);
    ...
    public async Task Failed_transaction_cannot_rollback_to_savepoint_with_custom_timeout()
    {
        ...
        using var cmd = new GaussDBCommand("SELECT unknown_thing", conn);
        cmd.CommandTimeout = 1;
        try
        {
            cmd.ExecuteScalar();
    ...
    public async Task Failed_transaction_on_close_with_custom_timeout()
    {
        ...
        using (var badCmd = new GaussDBCommand("SEL", conn))
        {
            badCmd.CommandTimeout = GaussDBCommand.DefaultTimeout + 1;
    ...
    public async Task Transaction_on_recycled_connection()
    {
        ...
        await using var conn = new GaussDBConnection(ConnectionString + $";Application Name={GetUniqueIdentifier(nameof(Transaction_on_recycled_connection))}");
        conn.Open();
    ...
    public async Task Transaction_not_supported()
    {
        ...
        GaussDBDatabaseInfo.RegisterFactory(new NoTransactionDatabaseInfoFactory());
        try
        {
    ...
    [Test, IssueLink("https://github.com/gaussdb/efcore.pg/issues/1593")]
    ...
    class NoTransactionDatabaseInfoFactory : IGaussDBDatabaseInfoFactory
    {
        public async Task<GaussDBDatabaseInfo?> Load(GaussDBConnector conn, GaussDBTimeout timeout, bool async)
        {
    ...
    class NoTransactionDatabaseInfo : PostgresDatabaseInfo
    {
        ...
        internal NoTransactionDatabaseInfo(GaussDBConnector conn) : base(conn) {}
    }
    ...
    public void Bug184_Rollback_fails_on_aborted_transaction()
    {
        ...
        using var connTimeoutChanged = new GaussDBConnection(csb.ToString());
        connTimeoutChanged.Open();
        using var t = connTimeoutChanged.BeginTransaction();
        try {
            var command = new GaussDBCommand("select count(*) from dta", connTimeoutChanged, t);
