    ...
    public Task CreatePgOutputReplicationSlot()
    {
        ...
        return SafeReplicationTest(
            async (slotName, _) =>
            {
                ...
                using var cmd =
                    new GaussDBCommand($"SELECT * FROM pg_replication_slots WHERE slot_name = '{options.Name}'",
                        c);
...
");
    ...
    public Task Stream()
    {
        ...
        return SafePgOutputReplicationTest(
            async (slotName, tableName, publicationName) =>
            {
                ...
                using (var command = new GaussDBCommand($"INSERT INTO {tableName} VALUES ($1)", c))
                {
    ...
    [GaussDBTypes.PgName("descriptor")]
    public class Descriptor
    {
        [GaussDBTypes.PgName("id")]
        public long Id { get; set; }

        [GaussDBTypes.PgName("name")]
...
CREATE PUBLICATION {publicationName} FOR TABLE {tableName};");
                ...
                GaussDBConnection.GlobalTypeMapper.MapComposite<Descriptor>("descriptor");
