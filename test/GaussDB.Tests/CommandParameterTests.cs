...
public class CommandParameterTests(MultiplexingMode multiplexingMode) : MultiplexingTestBase(multiplexingMode)
{
    ...
    public async Task Input_and_output_parameters(CommandBehavior behavior)
    {
        ...
        using var cmd = new GaussDBCommand("SELECT @c-1 AS c, @a+2 AS b", conn);
        cmd.Parameters.Add(new GaussDBParameter("a", 3));
        var b = new GaussDBParameter { ParameterName = "b", Direction = ParameterDirection.Output };
        cmd.Parameters.Add(b);
        var c = new GaussDBParameter { ParameterName = "c", Direction = ParameterDirection.InputOutput, Value = 4 };
    ...
    [Test]
    public async Task Send_GaussDBDbType_Unknown([Values(PrepareOrNot.NotPrepared, PrepareOrNot.Prepared)] PrepareOrNot prepare)
    {
        if (prepare == PrepareOrNot.Prepared && IsMultiplexing)
            return;

        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand("SELECT @p::TIMESTAMP", conn);
        cmd.CommandText = "SELECT @p::TIMESTAMP";
        cmd.Parameters.Add(new GaussDBParameter("p", GaussDBDbType.Unknown) { Value = "2008-1-1" });
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new GaussDBCommand("SELECT $1; SELECT $1", conn);
        cmd.Parameters.Add(new GaussDBParameter { GaussDBDbType = GaussDBDbType.Integer, Value = 8 });
    ...
    public async Task Unreferenced_positional_parameter_works()
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new GaussDBCommand("SELECT 1", conn);
        cmd.Parameters.Add(new GaussDBParameter { Value = 8 });
        Assert.That(await cmd.ExecuteScalarAsync(), Is.EqualTo(1));
    }
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new GaussDBCommand("SELECT $1, @p", conn);
        cmd.Parameters.Add(new GaussDBParameter { Value = 8 });
        cmd.Parameters.Add(new GaussDBParameter { ParameterName = "p", Value = 9 });
    ...
    public async Task Positional_output_parameters_are_not_supported()
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new GaussDBCommand("SELECT $1", conn);
        cmd.Parameters.Add(new GaussDBParameter { Value = 8, Direction = ParameterDirection.InputOutput });
        Assert.That(() => cmd.ExecuteNonQueryAsync(), Throws.Exception.TypeOf<NotSupportedException>());
    }
    ...
    public void Parameters_get_name()
    {
        ...
        // Add parameters.
        command.Parameters.Add(new GaussDBParameter(":Parameter1", DbType.Boolean));
        command.Parameters.Add(new GaussDBParameter(":Parameter2", DbType.Int32));
        command.Parameters.Add(new GaussDBParameter(":Parameter3", DbType.DateTime));
        command.Parameters.Add(new GaussDBParameter("Parameter4", DbType.DateTime));
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand("SELECT @p1, @p1", conn);
        cmd.Parameters.AddWithValue("@p1", 8);
        using var reader = await cmd.ExecuteReaderAsync();
        reader.Read();
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand("SELECT @p1, @p2, @p3, @p4", conn);
        cmd.Parameters.Add(new GaussDBParameter<int>("p1", 8));
        cmd.Parameters.Add(new GaussDBParameter<short>("p2", 8) { GaussDBDbType = GaussDBDbType.Integer });
        cmd.Parameters.Add(new GaussDBParameter<string>("p3", "hello"));
        cmd.Parameters.Add(new GaussDBParameter<char[]>("p4", ['f', 'o', 'o']));
        using var reader = await cmd.ExecuteReaderAsync();
        reader.Read();
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new GaussDBCommand("SELECT @p1::TEXT", conn);
        cmd.Parameters.Add(
            genericParam
                ? new GaussDBParameter<object?>("p1", null)
                : new GaussDBParameter("p1", null)
        );

        Assert.That(async () => await cmd.ExecuteReaderAsync(),
            Throws.Exception
                .TypeOf<InvalidOperationException>()
                .With.Message.EqualTo("Parameter 'p1' must have either its GaussDBDbType or its DataTypeName or its Value set."));
    }

    [Test]
    public async Task Object_generic_param_does_runtime_lookup()
    {
        await AssertTypeWrite<object>(1, "1", "integer", GaussDBDbType.Integer, DbType.Int32, DbType.Int32, isDefault: false,
            isGaussDBDbTypeInferredFromClrType: true, skipArrayCheck: true);
        await AssertTypeWrite<object>(new[] {1, 1}, "{1,1}", "integer[]", GaussDBDbType.Integer | GaussDBDbType.Array, isDefault: false,
            isGaussDBDbTypeInferredFromClrType: true, skipArrayCheck: true);
    }
    ...
    public async Task Object_generic_parameter_works()
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new GaussDBCommand("SELECT $1", conn);
        cmd.Parameters.Add(new GaussDBParameter<object> { GaussDBDbType = GaussDBDbType.Integer, Value = 8 });
        Assert.That(await cmd.ExecuteScalarAsync(), Is.EqualTo(8));
    }
}
