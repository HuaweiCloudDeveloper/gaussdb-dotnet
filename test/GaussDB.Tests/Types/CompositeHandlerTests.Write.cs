...
{
    async Task Write<T>(Action<GaussDBDataReader, T> assert, string? schema = null)
    ...
    async Task Write<T>(T composite, Action<GaussDBDataReader, T>? assert = null, string? schema = null)
        where T : IComposite
    {
        await using var dataSource = await OpenAndMapComposite(composite, schema, nameof(Write), out var _);
        await using var connection = await dataSource.OpenConnectionAsync();
        {
            await using var command = new GaussDBCommand("SELECT (@c).*", connection);
        ...
        {
            await using var command = new GaussDBCommand("SELECT (@arrayc)[1].*", connection);
