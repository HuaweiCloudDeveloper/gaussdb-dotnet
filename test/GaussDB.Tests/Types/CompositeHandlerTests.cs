...
{
    Task<GaussDBDataSource> OpenAndMapComposite<T>(T composite, string? schema, string nameSuffix, out string nameQualified)
        where T : IComposite
    {
        var nameTranslator = new GaussDBSnakeCaseNameTranslator();
        ...
        async Task<GaussDBDataSource> OpenAndMapCompositeCore(string nameQualified)
        {
