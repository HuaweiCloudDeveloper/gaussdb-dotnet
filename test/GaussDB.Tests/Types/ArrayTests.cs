...
public class ArrayTests(MultiplexingMode multiplexingMode) : MultiplexingTestBase(multiplexingMode)
{
    ...
    [
        new TestCaseData(new[] { 1, 2, 3 }, "{1,2,3}", "integer[]", GaussDBDbType.Integer | GaussDBDbType.Array)
            .SetName("Integer_array"),
        new TestCaseData(Array.Empty<int>(), "{}", "integer[]", GaussDBDbType.Integer | GaussDBDbType.Array)
            .SetName("Empty_array"),
        new TestCaseData(new[,] { { 1, 2, 3 }, { 7, 8, 9 } }, "{{1,2,3},{7,8,9}}", "integer[]", GaussDBDbType.Integer | GaussDBDbType.Array)
            .SetName("Two_dimensional_array"),
        new TestCaseData(new[] { [1, 2], new byte[] { 3, 4 } }, """{"\\x0102","\\x0304"}""", "bytea[]", GaussDBDbType.Bytea | GaussDBDbType.Array)
            .SetName("Bytea_array")
    ];

    [Test, TestCaseSource(nameof(ArrayTestCases))]
    public Task Arrays<T>(T array, string sqlLiteral, string pgTypeName, GaussDBDbType? gaussdbDbType)
        => AssertType(array, sqlLiteral, pgTypeName, gaussdbDbType);
    ...
    public async Task NullableInts()
    {
        ...
        var dataSourceBuilder = new GaussDBDataSourceBuilder(connectionStringBuilder.ToString());
        await using var dataSource = dataSourceBuilder.Build();

        await AssertType(dataSource, new int?[] { 1, 2, null, 3 }, "{1,2,NULL,3}", "integer[]", GaussDBDbType.Integer | GaussDBDbType.Array);
    }
...
""", conn);
    ...
    public async Task Write_empty_multidimensional_array()
        => await AssertTypeWrite(new int[0, 0], "{}", "integer[]", GaussDBDbType.Integer | GaussDBDbType.Array);

    [Test]
    public async Task Generic_List()
        => await AssertType(
            new List<int> { 1, 2, 3 }, "{1,2,3}", "integer[]", GaussDBDbType.Integer | GaussDBDbType.Array, isDefaultForReading: false);

    [Test]
    public async Task Write_IList_implementation()
        => await AssertTypeWrite(
            ImmutableArray.Create(1, 2, 3), "{1,2,3}", "integer[]", GaussDBDbType.Integer | GaussDBDbType.Array);
    ...
    public async Task Generic_IList()
    {
        ...
        cmd.Parameters.Add(new GaussDBParameter<IList<int>>("p1", GaussDBDbType.Array | GaussDBDbType.Integer) { TypedValue = expected });
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new GaussDBCommand("SELECT ARRAY[[1], [2]]", conn);
    ...
    public async Task Read_null_as_non_nullable_array_throws()
    {
        ...
        cmd.Parameters.AddWithValue("p1", GaussDBDbType.Array | GaussDBDbType.Integer, expected);
    ...
    public async Task Long_one_dimensional()
    {
        ...
        var p = new GaussDBParameter {ParameterName = "p", Value = expected};
    ...
    public async Task Strings_with_null()
    {
        ...
        await using var cmd = new GaussDBCommand("SELECT @p", conn);
        var p = new GaussDBParameter("p", GaussDBDbType.Array | GaussDBDbType.Text) {Value = expected};
        cmd.Parameters.Add(p);
        await using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SequentialAccess);
        reader.Read();
    ...
    public async Task Jagged_arrays_not_supported()
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new GaussDBCommand("SELECT @p1", conn);
        cmd.Parameters.AddWithValue("p1", GaussDBDbType.Array | GaussDBDbType.Integer, new[] { [8], new[] { 8, 10 } });
    ...
    public async Task Array_of_domain()
    {
        ...
        await using var cmd = new GaussDBCommand("SELECT @p1::posint[], @p2::posint[][]", conn);
        var oneDim = new[] { 1, 3, 5, 9 };
        var twoDim = new[,] { { 1, 3 }, { 5, 9 } };
        cmd.Parameters.AddWithValue("p1", GaussDBDbType.Integer | GaussDBDbType.Array, oneDim);
        cmd.Parameters.AddWithValue("p2", GaussDBDbType.Integer | GaussDBDbType.Array, twoDim);
...
""");
        await conn.ReloadTypesAsync();
        await using var cmd = new GaussDBCommand("SELECT @p1::int_array_1d, @p2::int_array_2d", conn);
        var oneDim = new[] { 1, 3, 5, 9 };
        var twoDim = new[,] { { 1, 3 }, { 5, 9 } };
        cmd.Parameters.AddWithValue("p1", GaussDBDbType.Integer | GaussDBDbType.Array, oneDim);
        cmd.Parameters.AddWithValue("p2", GaussDBDbType.Integer | GaussDBDbType.Array, twoDim);
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new GaussDBCommand("SELECT '{}'::INT[], '{}'::INT[]", conn);
        await using var reader = await cmd.ExecuteReaderAsync();
    ...
    [Test]
    public async Task Arrays_not_supported_by_default_on_GaussDBSlimSourceBuilder()
    {
    ...
    public async Task GaussDBSlimSourceBuilder_EnableArrays()
    {
        ...
        await AssertType(dataSource, new[] { 1, 2, 3 }, "{1,2,3}", "integer[]", GaussDBDbType.Integer | GaussDBDbType.Array);
    }
}
