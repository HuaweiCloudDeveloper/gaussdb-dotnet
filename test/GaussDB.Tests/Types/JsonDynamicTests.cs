...
public class JsonDynamicTests : MultiplexingTestBase
{
    ...
    public async Task Write_jsonobject_array_without_gaussdbdbtype()
    {
        // By default we map JsonObject to jsonb
        if (!IsJsonb)
            return;
    ...
    public async Task Poco_does_not_stomp_GetValue_string()
    {
        ...
        await using var cmd = new GaussDBCommand($"SELECT '{sqlLiteral}'::{(IsJsonb ? "jsonb" : "json")}", conn);
    ...
    public async Task Poco_polymorphic_mapping()
    {
        ...
        await AssertTypeWrite(dataSource, value, sql, PostgresType, GaussDBDbType, isGaussDBDbTypeInferredFromClrType: false);
    ...
    public async Task Poco_polymorphic_mapping_read_parents()
    {
        ...
        await AssertTypeWrite<WeatherForecast>(dataSource, value, sql, PostgresType, GaussDBDbType,
            isGaussDBDbTypeInferredFromClrType: false);
    ...
    public async Task Poco_unspecified_polymorphic_mapping()
    {
        ...
        await AssertTypeWrite(dataSource, value, sql, PostgresType, GaussDBDbType, isDefault: false);
    ...
    public JsonDynamicTests(MultiplexingMode multiplexingMode, GaussDBDbType gaussdbDbType)
