...
public class RecordTests(MultiplexingMode multiplexingMode) : MultiplexingTestBase(multiplexingMode)
{
    ...
    public async Task Read_Record_as_object_array()
    {
        ...
        await using var cmd = new GaussDBCommand($"SELECT {recordLiteral}, ARRAY[{recordLiteral}, {recordLiteral}]", conn);
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new GaussDBCommand("SELECT ('one'::TEXT, 2)", conn);
        await using var reader = await cmd.ExecuteReaderAsync();
        reader.Read();
    ...
    public async Task As_ValueTuple_supported_only_with_EnableRecordsAsTuples()
    {
        ...
        await using var command = new GaussDBCommand("SELECT (1, 'foo')::record", connection);
    ...
    [Test]
    public async Task Records_not_supported_by_default_on_GaussDBSlimSourceBuilder()
    {
    ...
    [Test]
    public async Task GaussDBSlimSourceBuilder_EnableRecords()
    {
        var dataSourceBuilder = new GaussDBSlimDataSourceBuilder(ConnectionString);
