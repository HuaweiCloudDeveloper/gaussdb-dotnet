...
public class BitStringTests(MultiplexingMode multiplexingMode) : MultiplexingTestBase(multiplexingMode)
{
    ...
    public async Task BitArray(string sqlLiteral)
    {
        ...
        await AssertType(bitArray, sqlLiteral, "bit varying", GaussDBDbType.Varbit);

        if (len > 0)
            await AssertType(bitArray, sqlLiteral, $"bit({len})", GaussDBDbType.Bit, isDefaultForWriting: false);
    }
    ...
    public Task BitVector32()
        => AssertType(
            new BitVector32(4), "00000000000000000000000000000100", "bit varying", GaussDBDbType.Varbit, isDefaultForReading: false);
    ...
    [Test]
    public Task Bool()
        => AssertType(true, "1", "bit(1)", GaussDBDbType.Bit, isDefault: false);
    ...
    public async Task Array()
    {
        ...
        var p = new GaussDBParameter("p", GaussDBDbType.Array | GaussDBDbType.Varbit) { Value = expected };
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand("SELECT @p::BIT(1)[]", conn);
        var expected = new[] { true, false };
        var p = new GaussDBParameter("p", GaussDBDbType.Array | GaussDBDbType.Bit) {Value = expected};
        cmd.Parameters.Add(p);
        p.Value = expected;
        using var reader = await cmd.ExecuteReaderAsync();
        reader.Read();
    ...
    public Task As_string()
        => AssertType("010101", "010101", "bit varying", GaussDBDbType.Varbit, isDefault: false);
