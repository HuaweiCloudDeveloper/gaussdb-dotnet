...
class SqlQueryParserTests
{
    ...
    {
        var parameters = new GaussDBParameter[] { new(":p1", "foo"), new(":p2", "foo") };
    ...
    {
        var p = new GaussDBParameter(":a.parameter", "foo");
    ...
    {
        var results = ParseCommand(sql, new GaussDBParameter(":param", "foo"));
    ...
    {
        var p = new GaussDBParameter(":param", "foo");
    ...
    {
        var p = new GaussDBParameter("漢字", "foo");
...
2", TestName = "LineComment")]
    ...
    {
        var parameters = new GaussDBParameter[]
        {
    ...
    {
        var p = new GaussDBParameter("p", DbType.String) { Direction = ParameterDirection.Output };
    ...
    public void Reduce_number_of_statements()
    {
        ...
        var cmd = new GaussDBCommand("SELECT 1; SELECT 2");
    ...
    List<GaussDBBatchCommand> ParseCommand(string sql, params GaussDBParameter[] parameters)
        => ParseCommand(sql, parameters, standardConformingStrings: true);

    List<GaussDBBatchCommand> ParseCommand(string sql, GaussDBParameter[] parameters, bool standardConformingStrings)
    {
        var cmd = new GaussDBCommand(sql);
