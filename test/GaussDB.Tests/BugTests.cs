...
public class BugTests : TestBase
{
    ...
    public void SequentialNullCheckOnNonFirstField()
    {
        ...
        using var cmd = new GaussDBCommand("SELECT 'X', NULL", conn);
    ...
    {
        using var conn = OpenConnection();
        using var command = new GaussDBCommand("SELECT 1, NULL", conn);
    ...
    public async Task Bug3600()
    {
        ...
        await serverMock
            .WriteCopyInResponse()
            .FlushAsync();
        var ex = Assert.ThrowsAsync<GaussDBException>(async () =>
        {
    ...
    public async Task Bug1497()
    {
        ...
        await using var cmd = new GaussDBCommand($"SELECT * FROM {tableName}", conn);
    ...
    public void Bug1700()
        => Assert.That(() =>
        {
            ...
            while (reader.Read())
            {
                ...
                // As this exception unwinds the stack, it calls Dispose on the GaussDBTransaction
                // which then throws a GaussDBOperationInProgressException as it tries to rollback
    ...
    {
        using var conn = OpenConnection();
        using var cmd = new GaussDBCommand("INVALID SQL", conn);
        cmd.Parameters.Add(new GaussDBParameter { ParameterName = "p", Direction = ParameterDirection.Output });
    ...
    {
        using var conn = OpenConnection();
        using var cmd = new GaussDBCommand("SELECT 'hello', 'goodbye'", conn);
        using var reader = cmd.ExecuteReader();
        reader.Read();
    ...
    public async Task Bug1987()
    {
        ...
        for (var i = 0; i < 2; i++)
        {
            await using var cmd = new GaussDBCommand("SELECT @p", connection);
    ...
    public void Bug2003()
    {
        ...
        using var cmd = new GaussDBCommand($"SELECT 8 AS {longFieldName}", conn);
        using var reader = cmd.ExecuteReader(CommandBehavior.SequentialAccess);
        reader.Read();
    ...
    public async Task Bug2046()
    {
        ...
        using var conn = OpenConnection();
        using var cmd = new GaussDBCommand("SELECT @p = 64.27245::real, 64.27245::real, @p", conn);
    ...
    public void Bug1761()
    {
        ...
        for (var i = 0; i < 2; i++)
        {
            try
            {
                ...
                using (var cmd = new GaussDBCommand("SELECT 1", connection))
                {
                    cmd.CommandText = "select 1;";
                    cmd.ExecuteNonQuery();
                }
...
WHERE table_name LIKE @p0 escape '\' AND (is_updatable = 'NO') = @p1";
    ...
    public void Bug1285()
    {
        ...
        cmd.Parameters.Add(new GaussDBParameter("@1", Guid.NewGuid()));
        cmd.ExecuteNonQuery();
    }
...
(
  ...
  CONSTRAINT ""pk_OrganisatieQmo_Organisatie_QueryModelObjects_Imp"" PRIMARY KEY (""Id"")
    ...
    public async Task Chunked_string_write_buffer_encoding_space()
    {
        ...
        await binaryImporter.WriteAsync(almostBufferFillingString, GaussDBDbType.Text);
        ...
        await binaryImporter.WriteAsync(longStringStartingWithAforementionedUnicodeCharacter, GaussDBDbType.Text);
    ...
    public async Task Chunked_char_array_write_buffer_encoding_space()
    {
        ...
        // get encoded to multiple bytes
        var longStringStartingWithAforementionedUnicodeCharacter = unicodeCharacterThatEncodesToThreeBytesInUtf8 + new string('a', 10000);
        await binaryImporter.WriteAsync(longStringStartingWithAforementionedUnicodeCharacter.ToCharArray(), GaussDBDbType.Text);
...
$$;");
    ...
    public void Bug3117()
    {
        ...
        using (var conn = dataSource.OpenConnection())
        {
            var okCommand = new GaussDBCommand(OkCommand, conn);
            okCommand.Prepare();
            using (okCommand.ExecuteReader()) { }

            var errorCommand = new GaussDBCommand(ErrorCommand, conn);
    ...
    public async Task Bug3649()
    {
        ...
        await using (var importer = await conn.BeginBinaryImportAsync($"COPY {table} (value) FROM STDIN BINARY"))
        {
            ...
            await importer.WriteAsync(DBNull.Value, GaussDBDbType.Integer);
            await importer.StartRowAsync();
            await importer.WriteAsync(1, GaussDBDbType.Integer);
            await importer.StartRowAsync();
            await importer.WriteAsync(2, GaussDBDbType.Integer);
    ...
    public async Task UIThreadSynchronizationContext_deadlock()
    {
        ...
        using (var _ = syncContext.Enter())
        {
            ...
            await using var cmd = new GaussDBCommand("SELECT generate_series(1, 500000); SELECT @p", connection);
