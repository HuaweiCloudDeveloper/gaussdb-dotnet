...
$$ LANGUAGE 'plpgsql';");
    ...
    [Test]
    public void GaussDBException_with_async()
    {
        using var conn = OpenConnection();
    ...
    [Test]
    public void GaussDBException_IsTransient()
    {
        Assert.True(new GaussDBException("", new IOException()).IsTransient);
        Assert.True(new GaussDBException("", new SocketException()).IsTransient);
        Assert.True(new GaussDBException("", new TimeoutException()).IsTransient);
        Assert.False(new GaussDBException().IsTransient);
        Assert.False(new GaussDBException("", new Exception("Inner Exception")).IsTransient);
    }
