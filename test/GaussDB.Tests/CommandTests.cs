...
public class CommandTests(MultiplexingMode multiplexingMode) : MultiplexingTestBase(multiplexingMode)
{
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new GaussDBCommand($"SELECT repeat('X', {conn.Settings.WriteBufferSize}); SELECT @p", conn);
    ...
    public async Task Named_parameters_are_not_supported_when_EnableSqlParsing_is_disabled()
    {
        ...
        cmd.Parameters.Add(new GaussDBParameter("p", 8));
    ...
    public async Task Timeout_async_hard()
    {
        ...
        var builder = new GaussDBConnectionStringBuilder(ConnectionString) { CommandTimeout = 1 };
    ...
    {
        Assert.That(GaussDBConnector.MinimumInternalCommandTimeout, Is.Not.EqualTo(GaussDBCommand.DefaultTimeout));
        var timeout = GaussDBConnector.MinimumInternalCommandTimeout;
        await using var dataSource = CreateDataSource(csb => csb.CommandTimeout = timeout);
        await using var conn = await dataSource.OpenConnectionAsync();
        await using var command = new GaussDBCommand("SELECT 1", conn);
    ...
    public async Task Prepare_timeout_hard([Values] SyncOrAsync async)
    {
        ...
        var cmd = new GaussDBCommand("SELECT 1", conn);
        Assert.That(async () =>
            {
    ...
    public async Task Cancel_cross_command()
    {
        ...
        await using var cmd2 = new GaussDBCommand("SELECT 1", conn);
    ...
    public async Task Cursor_statement()
    {
        ...
        var i = 0;
        var command = new GaussDBCommand($"DECLARE TE CURSOR FOR SELECT * FROM {table}", conn);
    ...
    public async Task Cursor_move_RecordsAffected()
    {
        ...
        var command = new GaussDBCommand("DECLARE curs CURSOR FOR SELECT * FROM (VALUES (1), (2), (3)) as t", connection);
        command.ExecuteNonQuery();
    ...
    {
        using var conn = await OpenConnectionAsync();
        using (var cmd = new GaussDBCommand("SE", conn))
    ...
    public async Task SingleRow([Values(PrepareOrNot.NotPrepared, PrepareOrNot.Prepared)] PrepareOrNot prepare)
    {
        ...
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new GaussDBCommand("SELECT 1, 2 UNION SELECT 3, 4", conn);
        if (prepare == PrepareOrNot.Prepared)
            cmd.Prepare();
    ...
    public async Task ExecuteScalar()
    {
        await using var conn = await OpenConnectionAsync();
        var table = await CreateTempTable(conn, "name TEXT");
        await using var command = new GaussDBCommand($"SELECT name FROM {table}", conn);
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new GaussDBCommand { Connection = conn };
        var table = await CreateTempTable(conn, "name TEXT");
    ...
    {
        await using var conn = await OpenConnectionAsync();
        var cmd = new GaussDBCommand("SELECT 1, 2", conn);
        await cmd.ExecuteReaderAsync();
        cmd.Dispose();
        cmd = new GaussDBCommand("SELECT 3", conn);
        Assert.That(() => cmd.ExecuteScalarAsync(), Throws.Exception.TypeOf<GaussDBOperationInProgressException>());
    }
    ...
    public async Task Parameter_and_operator_unclear()
    {
        ...
        //a syntax error. See comment in GaussDBCommand.GetClearCommandText() on "usually-redundant parenthesis".
        await using var command = new GaussDBCommand("select :arr[2]", conn);
    ...
    {
        await using var conn = await OpenConnectionAsync();
        var command = new GaussDBCommand("select 3, 4 as param1, 5 as param2, 6;", conn);

        var p = new GaussDBParameter("param2", GaussDBDbType.Integer);
        p.Direction = ParameterDirection.Output;
        p.Value = -1;
        command.Parameters.Add(p);

        p = new GaussDBParameter("param1", GaussDBDbType.Integer);
        p.Direction = ParameterDirection.Output;
        p.Value = -1;
        command.Parameters.Add(p);

        p = new GaussDBParameter("p", GaussDBDbType.Integer);
        p.Direction = ParameterDirection.Output;
        p.Value = -1;
        command.Parameters.Add(p);
...
$$ LANGUAGE plpgsql;";
        ...
        var command = new GaussDBCommand(createFunction, conn);
        await command.ExecuteNonQueryAsync();

        command = new GaussDBCommand(sproc, conn);
        command.CommandType = CommandType.StoredProcedure;

        command.Parameters.Add(new GaussDBParameter("a", DbType.Int32));
        command.Parameters[0].Direction = ParameterDirection.Output;
        command.Parameters.Add(new GaussDBParameter("b", DbType.Boolean));
    ...
    public async Task Bug1010788_UpdateRowSource()
    {
        ...
        var command = new GaussDBCommand($"SELECT * FROM {table}", conn);
        Assert.AreEqual(UpdateRowSource.Both, command.UpdatedRowSource);

        var cmdBuilder = new GaussDBCommandBuilder();
        var da = new GaussDBDataAdapter(command);
    ...
    public async Task TableDirect()
    {
        ...
        using var cmd = new GaussDBCommand(table, conn) { CommandType = CommandType.TableDirect };
    ...
    public async Task Parameter_overflow_message_length_throws()
    {
        ...
        await using var dataSource = CreateDataSource();
        await using var conn = await dataSource.OpenConnectionAsync();
        await using var cmd = new GaussDBCommand("SELECT @a, @b, @c, @d, @e, @f, @g, @h", conn);
    ...
    public async Task Range_overflow_message_length_throws()
    {
        ...
        var range = new GaussDBRange<BigComposite>(composite, composite);
        cmd.Parameters.Add(new GaussDBParameter
        {
            Value = range,
            ParameterName = "a",
    ...
    {
        var cmd = new GaussDBCommand("SELECT 1");
    ...
    public async Task Many_parameters([Values(PrepareOrNot.NotPrepared, PrepareOrNot.Prepared)] PrepareOrNot prepare)
    {
        ...
        using var cmd = new GaussDBCommand { Connection = conn };
        var sb = new StringBuilder($"INSERT INTO {table} (some_column) VALUES ");
        for (var i = 0; i < ushort.MaxValue; i++)
        {
            var paramName = "p" + i;
            cmd.Parameters.Add(new GaussDBParameter(paramName, 8));
            if (i > 0)
                sb.Append(", ");
            sb.Append($"(@{paramName})");
        }
    ...
    [Test, Description("Makes sure that GaussDB doesn't attempt to send all data before the user can start reading. That would cause a deadlock.")]
    public async Task Batched_big_statements_do_not_deadlock()
    {
        ...
        for (var i = 0; i < 500; i++)
            sb.Append("SELECT @p;");
        using var cmd = new GaussDBCommand(sb.ToString(), conn);
        cmd.Parameters.AddWithValue("p", GaussDBDbType.Text, data);
        using var reader = await cmd.ExecuteReaderAsync();
        for (var i = 0; i < 500; i++)
        {
            reader.Read();
    ...
    public void Batched_small_then_big_statements_do_not_deadlock_in_sync_io()
    {
        ...
        using var cmd = new GaussDBCommand("SELECT generate_series(1, 500000); SELECT @p", conn);
        cmd.Parameters.AddWithValue("p", GaussDBDbType.Text, data);
        cmd.ExecuteNonQuery();
    }
    ...
    public async Task Bug3509()
    {
        ...
        var ex = Assert.ThrowsAsync<GaussDBException>(async () => await queryTask)!;
        Assert.That(ex.InnerException, Is.TypeOf<GaussDBException>()
            .With.InnerException.TypeOf<PostgresException>());
    }
    ...
    public async Task Prepare_with_positional_placeholders_after_named()
    {
        ...
        command.Parameters.Add(new() { GaussDBDbType = GaussDBDbType.Integer });
    ...
    public async Task Not_cancel_prepended_query([Values] bool failPrependedQuery)
    {
        ...
        await using var postmasterMock = PgPostmasterMock.Start(ConnectionString);
        var csb = new GaussDBConnectionStringBuilder(postmasterMock.ConnectionString)
        {
