...
#endif
public class GaussDBParameterCollectionTests
{
    readonly CompatMode _compatMode;
    const int LookupThreshold = GaussDBParameterCollection.LookupThreshold;

    [Test]
    public void Can_only_add_GaussDBParameter()
    {
        using var command = new GaussDBCommand();
    ...
    public void Clear()
    {
        ...
        var c1 = new GaussDBCommand();
        var c2 = new GaussDBCommand();
        c1.Parameters.Add(p);
    ...
    public void Hash_lookup_parameter_rename_bug()
    {
        ...
        {
            command.Parameters.AddWithValue(string.Format("p{0:00}", i + 1), GaussDBDbType.Text, string.Format("String parameter value {0}", i + 1));
        }
    ...
    public void Remove_duplicate_parameter([Values(LookupThreshold, LookupThreshold - 2)] int count)
    {
        ...
        {
            command.Parameters.AddWithValue(string.Format("p{0:00}", i + 1), GaussDBDbType.Text,
        ...
        command.Parameters.AddWithValue("P02", GaussDBDbType.Text, "String parameter value 2");
    ...
    public void Remove_case_differing_parameter([Values(LookupThreshold, LookupThreshold - 2)] int count)
    {
        ...
        using var command = new GaussDBCommand();
        command.Parameters.Add(new GaussDBParameter("PP0", 1));
        command.Parameters.Add(new GaussDBParameter("Pp0", 1));
        for (var i = 0; i < count - 2; i++)
            command.Parameters.Add(new GaussDBParameter($"pp{i}", i));
    ...
    public void Correct_index_returned_for_duplicate_ParameterName([Values(LookupThreshold, LookupThreshold - 2)] int count)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            command.Parameters.AddWithValue(string.Format("parameter{0:00}", i + 1), GaussDBDbType.Text, string.Format("String parameter value {0}", i + 1));
        }
        ...
        command.Parameters.AddWithValue("Parameter02", GaussDBDbType.Text, "String parameter value 2");

        // Insert another case insensitive before the original.
        command.Parameters.Insert(0, new GaussDBParameter("ParameteR02", GaussDBDbType.Text) { Value = "String parameter value 2" });
    ...
    public void Finds_case_insensitive_lookups([Values(LookupThreshold, LookupThreshold - 2)] int count)
    {
        ...
        for (var i = 0; i < count; i++)
            parameters.Add(new GaussDBParameter($"p{i}", i));
    ...
    public void Throws_on_indexer_mismatch([Values(LookupThreshold, LookupThreshold - 2)] int count)
    {
        ...
        Assert.DoesNotThrow(() =>
        {
            command.Parameters["p1"] = new GaussDBParameter("p1", 1);
            command.Parameters["p1"] = new GaussDBParameter("P1", 1);
        });

        Assert.Throws<ArgumentException>(() =>
        {
            command.Parameters["p1"] = new GaussDBParameter("p2", 1);
        });
    }
    ...
    public void Positional_parameter_lookup_returns_first_match([Values(LookupThreshold, LookupThreshold - 2)] int count)
    {
        ...
        var parameters = command.Parameters;
        for (var i = 0; i < count; i++)
            parameters.Add(new GaussDBParameter(GaussDBParameter.PositionalName, i));
    ...
    {
        using var cmd = new GaussDBCommand("SELECT $1, $2");
        var p = new GaussDBParameter("", "Hello world");
    ...
    public void IndexOf_falls_back_to_first_insensitive_match([Values] bool manyParams)
    {
        ...
        parameters.Add(new GaussDBParameter("foo", 8));
        parameters.Add(new GaussDBParameter("bar", 8));
        parameters.Add(new GaussDBParameter("BAR", 8));
    ...
    public void IndexOf_matches_all_parameter_syntaxes()
    {
        ...
        parameters.Add(new GaussDBParameter("@foo0", 8));
        parameters.Add(new GaussDBParameter(":foo1", 8));
        parameters.Add(new GaussDBParameter("foo2", 8));
    ...
    public void Cloning_succeeds([Values(LookupThreshold, LookupThreshold - 2)] int count)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            command.Parameters.Add(new GaussDBParameter());
        }
    ...
    public void Clean_name()
    {
        ...
        Assert.AreEqual(GaussDBParameter.PositionalName, param.ParameterName);
    }
    ...
    public void Clone_sets_correct_collection()
    {
        ...
        cmd.Parameters.Add(new GaussDBParameter<int> { TypedValue = 42 });
    ...
    public GaussDBParameterCollectionTests(CompatMode compatMode)
    {
        _compatMode = compatMode;

#if DEBUG
        GaussDBParameterCollection.TwoPassCompatMode = compatMode == CompatMode.TwoPass;
#else
        if (compatMode == CompatMode.TwoPass)
            Assert.Ignore("Cannot test case-insensitive GaussDBParameterCollection behavior in RELEASE");
#endif
    }
