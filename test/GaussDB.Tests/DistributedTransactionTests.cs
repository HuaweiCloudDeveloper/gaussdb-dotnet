...
public class DistributedTransactionTests : TestBase
{
    ...
    [Explicit("Fails on Appveyor (https://ci.appveyor.com/project/roji/gaussdb/build/3.3.0-250)")]
...
Exception {2}",
    ...
    int GetNumberOfPreparedTransactions()
    {
        ...
        using (var cmd = new GaussDBCommand("SELECT COUNT(*) FROM pg_prepared_xacts WHERE database = @database", conn))
        {
            cmd.Parameters.Add(new GaussDBParameter("database", conn.Database));
            return (int)(long)cmd.ExecuteScalar()!;
        }
    }

    void AssertNumberOfRows(GaussDBConnection connection, string table, int expected)
    ...
    GaussDBDataSource EnlistOnDataSource { get; set; } = default!;

    GaussDBDataSource EnlistOffDataSource { get; set; } = default!;
...
-------------"));
    ...
    public void OneTimeSetUp()
    {
        ...
        using (var cmd = new GaussDBCommand("SELECT gid FROM pg_prepared_xacts WHERE database=@database", connection))
        {
            cmd.Parameters.AddWithValue("database", new GaussDBConnectionStringBuilder(ConnectionString).Database!);
    ...
    internal static string CreateTempTable(GaussDBConnection conn, string columns)
    {
