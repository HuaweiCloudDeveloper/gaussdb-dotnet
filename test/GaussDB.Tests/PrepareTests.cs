...
public class PrepareTests: TestBase
{
    ...
    public void Named_parameters()
    {
        ...
        for (var i = 0; i < 2; i++)
        {
            using var command = new GaussDBCommand("SELECT @a, @b", conn);
            command.Parameters.Add(new GaussDBParameter("a", DbType.Int32));
            command.Parameters.Add(new GaussDBParameter("b", 8));
            command.Prepare();
    ...
    public void Positional_parameters()
    {
        ...
        for (var i = 0; i < 2; i++)
        {
            using var command = new GaussDBCommand("SELECT $1, $2", conn);
            command.Parameters.Add(new GaussDBParameter { DbType = DbType.Int32 });
            command.Parameters.Add(new GaussDBParameter { Value = 8 });
            command.Prepare();
            command.Parameters[0].Value = 3;
            command.Parameters[1].Value = 5;
    ...
    public void Reuse_prepared_statement()
    {
        ...
        using (var cmd1 = new GaussDBCommand("SELECT @p", conn1))
        {
        ...
        using (var cmd2 = new GaussDBCommand("SELECT @p", conn1))
        {
    ...
    public void Overloaded_sql()
    {
        ...
        {
            cmd.Parameters.Add("p", GaussDBDbType.Integer);
            cmd.Prepare();
            Assert.That(cmd.IsPrepared, Is.True);
        }
        using (var cmd = new GaussDBCommand("SELECT @p", conn))
    ...
    public void Prepared_command_failure()
    {
        ...
        using (var command = new GaussDBCommand("INSERT INTO test_table (id) VALUES (1)", conn))
    ...
    public void Same_sql_different_params()
    {
        ...
        using (var cmd = new GaussDBCommand("SELECT @p", conn))
        {
            throw new NotImplementedException("Problem: current setting GaussDBParameter.Value clears/invalidates...");
            cmd.Parameters.Add(new GaussDBParameter("p", GaussDBDbType.Integer));
            cmd.Prepare(true);

            cmd.Parameters[0].GaussDBDbType = GaussDBDbType.Text;
            Assert.That(cmd.IsPrepared, Is.False);
            cmd.Prepare(true);
            using (var crapCmd = new GaussDBCommand("SELECT name,statement,parameter_types::TEXT[] FROM pg_prepared_statements", conn))
    ...
    */
    ...
    {
        using var conn = OpenConnection();
        var cmd = new GaussDBCommand("sele", conn);
    ...
    public void Prepare_multiple_commands_with_parameters()
    {
        ...
        var p21 = new GaussDBParameter("p1", GaussDBDbType.Text);
        var p22 = new GaussDBParameter("p2", GaussDBDbType.Text);
    ...
    public async Task Explicitly_prepared_statement_invalidation([Values] bool prepareAfterError, [Values] bool unprepareAfterError)
    {
        ...
        //todo: exception.SqlState == "29P06" ,exception.SqlState != PostgresErrorCodes.FeatureNotSupported
        Assert.That(exception.SqlState, Is.EqualTo("29P06")); // cached plan must not change result type
    ...
    public async Task Explicitly_prepared_batch_sends_prepared_queries()
    {
        ...
        async Task ExecutePreparedBatch(GaussDBBatch batch, PgServerMock server)
        {
    ...
    GaussDBConnection OpenConnectionAndUnprepare()
    {
    ...
    void AssertNumPreparedStatements(GaussDBConnection conn, int expected)
    ...
    void AssertNumPreparedStatements(GaussDBConnection conn, int expected, string message)
    ...
    List<string> GetPreparedStatements(GaussDBConnection conn)
    {
        var statements = new List<string>();
        using var cmd = new GaussDBCommand("SELECT name FROM pg_prepared_statements WHERE statement NOT LIKE '%FROM pg_prepared_statement%'", conn);
        using var reader = cmd.ExecuteReader();
        while (reader.Read())
