...
");
        ...
        Assert.That(intRow["ProviderDbType"], Is.EqualTo((int)GaussDBDbType.Integer));
        ...
        Assert.That(textRow["ProviderDbType"], Is.EqualTo((int)GaussDBDbType.Text));
        ...
        Assert.That(numericRow["ProviderDbType"], Is.EqualTo((int)GaussDBDbType.Numeric));
        ...
        Assert.That(intArrayRow["ProviderDbType"], Is.EqualTo((int)(GaussDBDbType.Integer | GaussDBDbType.Array)));
        ...
        //Assert.That(domainRow["ProviderDbType"], Is.EqualTo((int)GaussDBDbType.Text));
    ...
    public async Task ParameterMarkerFormat()
    {
        ...
        command.Parameters.Add(new GaussDBParameter(parameterName, 4));
...
CREATE TABLE {table} (color {schema}.{enumName});");
    ...
    {
        await using var dataSource = new GaussDBSlimDataSourceBuilder(ConnectionString).Build();
        await using var conn = await dataSource.OpenConnectionAsync();
        Assert.That(() => GetSchema(conn, DbMetaDataCollectionNames.DataTypes), Throws.Nothing);
    }

    // ReSharper disable MethodHasAsyncOverload
    async Task<DataTable> GetSchema(GaussDBConnection conn)
        => IsAsync ? await conn.GetSchemaAsync() : conn.GetSchema();

    async Task<DataTable> GetSchema(GaussDBConnection conn, string collectionName)
        => IsAsync ? await conn.GetSchemaAsync(collectionName) : conn.GetSchema(collectionName);

    async Task<DataTable> GetSchema(GaussDBConnection conn, string collectionName, string?[] restrictions)
