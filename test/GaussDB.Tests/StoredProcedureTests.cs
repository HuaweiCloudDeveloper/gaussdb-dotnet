...
END$$");
    ...
    public async Task Batch_positional_parameters_works()
    {
        ...
        await using var batch = new GaussDBBatch(connection, transaction)
        {
            BatchCommands =
            {
                new(tempname)
        ...
        await using var batch = new GaussDBBatch(connection, transaction)
        {
            BatchCommands =
            {
                new(sproc)
                {
                    ...
                    {
                        new() { Value = "" },
                        new() { GaussDBDbType = GaussDBDbType.Bigint, Direction = ParameterDirection.Output }
                    }
                },
                new(sproc)
                {
...
$$ LANGUAGE plpgsql");
        ...
        await using var command = new GaussDBCommand(sproc, conn) { CommandType = CommandType.StoredProcedure };
        ...
        Assert.That(command.Parameters[0].GaussDBDbType, Is.EqualTo(GaussDBDbType.Integer));
        ...
        Assert.That(command.Parameters[1].GaussDBDbType, Is.EqualTo(GaussDBDbType.Text));
        ...
        Assert.That(command.Parameters[2].GaussDBDbType, Is.EqualTo(GaussDBDbType.Integer));
    ...
    public async Task DeriveParameters_procedure_in_only()
    {
        ...
        await using var cmd = new GaussDBCommand(sproc, conn) { CommandType = CommandType.StoredProcedure };
    ...
    public async Task DeriveParameters_procedure_with_case_sensitive_name()
    {
        ...
        try
        {
            await using var command = new GaussDBCommand(@"""ProcedureCaseSensitive""", conn) { CommandType = CommandType.StoredProcedure };
            GaussDBCommandBuilder.DeriveParameters(command);
    ...
    public async Task DeriveParameters_dot_character_in_function_name()
    {
        ...
        try
        {
            await using var command = new GaussDBCommand(@"""My.Dotted.Procedure""", conn) { CommandType = CommandType.StoredProcedure };
            GaussDBCommandBuilder.DeriveParameters(command);
            Assert.AreEqual(GaussDBDbType.Integer, command.Parameters[0].GaussDBDbType);
            Assert.AreEqual(GaussDBDbType.Text, command.Parameters[1].GaussDBDbType);
        }
    ...
    {
        await using var conn = await OpenConnectionAsync();
        var invalidCommandName = new GaussDBCommand("invalidprocedurename", conn) { CommandType = CommandType.StoredProcedure };
...
SET search_path TO {schema2};");
        await using var command = new GaussDBCommand("redundantsproc", conn) { CommandType = CommandType.StoredProcedure };
        GaussDBCommandBuilder.DeriveParameters(command);
...
RESET search_path;");
        await using var command = new GaussDBCommand("schema1sproc", conn) { CommandType = CommandType.StoredProcedure };
...
SET search_path TO {schema1}, {schema2};");
        var command = new GaussDBCommand("redundantsproc", conn) { CommandType = CommandType.StoredProcedure };
        Assert.That(() => GaussDBCommandBuilder.DeriveParameters(command),
            Throws.Exception.TypeOf<PostgresException>()
