...
public class ConnectionTests(MultiplexingMode multiplexingMode) : MultiplexingTestBase(multiplexingMode)
{
    ...
    public async Task Broken_lifecycle([Values] bool openFromClose)
    {
        ...
        Assert.AreEqual(1, conn.GaussDBDataSource.Statistics.Total);
    ...
    {
        var csb = new GaussDBConnectionStringBuilder(ConnectionString) { Port = 44444, Pooling = pooled };
        using (var conn = new GaussDBConnection(csb)) {
    ...
    public async Task Unix_domain_socket()
    {
        ...
        var port = new GaussDBConnectionStringBuilder(ConnectionString).Port;
    ...
    {
        using var conn = new GaussDBConnection();
    ...
    {
        await using var c = new GaussDBConnection(ConnectionString);
    ...
    class TestDbInfo : GaussDBDatabaseInfo
    {
        ...
        public new static Version ParseServerVersion(string versionString)
            => GaussDBDatabaseInfo.ParseServerVersion(versionString);
    }
    ...
    {
        var conn = new GaussDBConnection();
        Assert.That(conn.ConnectionTimeout, Is.EqualTo(GaussDBConnectionStringBuilder.DefaultTimeout));
    ...
    {
        var conn = new GaussDBConnection(null);
        Assert.That(conn.ConnectionString, Is.SameAs(string.Empty));
        Assert.That(() => conn.Open(), Throws.Exception.TypeOf<InvalidOperationException>());
    }
    ...
    {
        var csb = new GaussDBConnectionStringBuilder(ConnectionString) { Database = null };
        await using var conn = new GaussDBConnection(csb.ToString());
    ...
    public async Task ChangeDatabase()
    {
        ...
        using var cmd = new GaussDBCommand("select current_database()", conn);
    ...
    {
        using var conn1 = new GaussDBConnection(ConnectionString);
    ...
    {
        var command = new GaussDBCommand();
...
LANGUAGE 'plpgsql'");
    ...
    public async Task Concurrent_use_throws()
    {
        ...
        using (await cmd.ExecuteReaderAsync())
            Assert.That(async () => await conn.ExecuteScalarAsync("SELECT 2"),
                Throws.Exception.TypeOf<GaussDBOperationInProgressException>()
                    .With.Property(nameof(GaussDBOperationInProgressException.CommandInProgress)).SameAs(cmd));
    ...
    public void PersistSecurityInfo_is_true([Values(true, false)] bool pooling)
    {
        ...
        using var conn = new GaussDBConnection(connString);
        var passwd = new GaussDBConnectionStringBuilder(conn.ConnectionString).Password;
        Assert.That(passwd, Is.Not.Null);
        conn.Open();
        Assert.That(new GaussDBConnectionStringBuilder(conn.ConnectionString).Password, Is.EqualTo(passwd));
    }
    ...
    public void No_password_without_PersistSecurityInfo([Values(true, false)] bool pooling)
    {
        ...
        using var conn = new GaussDBConnection(connString);
        var csb = new GaussDBConnectionStringBuilder(conn.ConnectionString);
        Assert.That(csb.PersistSecurityInfo, Is.False);
        Assert.That(csb.Password, Is.Not.Null);
        conn.Open();
        Assert.That(new GaussDBConnectionStringBuilder(conn.ConnectionString).Password, Is.Null);
    }
    ...
    public async Task Clone_with_PersistSecurityInfo([Values] bool async)
    {
        ...
        using var connWithPersist = new GaussDBConnection(connStringWithPersist);
    ...
    public async Task CloneWith_and_data_source_with_password([Values] bool async)
    {
        ...
        using var _ = CreateTempPool(new GaussDBConnectionStringBuilder(ConnectionString) { Password = null }, out var tempConnectionString);
    ...
    public void Clone()
    {
        ...
        using var conn2 = (GaussDBConnection)((ICloneable)conn).Clone();
    ...
    public async Task Clone_with_data_source()
    {
        ...
        await using var clonedConnection = (GaussDBConnection)((ICloneable)connection).Clone();

        Assert.That(clonedConnection.GaussDBDataSource, Is.SameAs(DataSource));
    ...
    {
        // GaussDB 3.0.0 to 3.0.4 prepended a rollback for the next time the connector is used, as an optimization.
    ...
    public async Task NoTypeLoading()
    {
        ...
        Assert.That(await conn.ExecuteScalarAsync("SELECT '[1,10)'::int4range"), Is.EqualTo(new GaussDBRange<int>(1, true, 10, false)));

        if (conn.PostgreSqlVersion >= new Version(14, 0))
        {
            var multirangeArray = (GaussDBRange<int>[])(await conn.ExecuteScalarAsync("SELECT '{[3,7), (8,]}'::int4multirange"))!;
            Assert.That(multirangeArray.Length, Is.EqualTo(2));
            Assert.That(multirangeArray[0], Is.EqualTo(new GaussDBRange<int>(3, true, false, 7, false, false)));
            Assert.That(multirangeArray[1], Is.EqualTo(new GaussDBRange<int>(9, true, false, 0, false, true)));
        }
        else
        {
            using var cmd = new GaussDBCommand("SELECT $1", conn)
            {
                Parameters = { new() { Value = DBNull.Value, GaussDBDbType = GaussDBDbType.IntegerMultirange } }
            };

            Assert.That(async () => await cmd.ExecuteScalarAsync(),
                Throws.Exception.TypeOf<NotSupportedException>()
                    .With.Message.EqualTo("The GaussDBDbType 'IntegerMultirange' isn't present in your database. You may need to install an extension or upgrade to a newer version."));
        }
    }
...
CREATE TABLE record (id INT)");
    ...
    public async Task Oversize_buffer()
    {
        ...
        using (var cmd = new GaussDBCommand($"SELECT '{bigString1}'", conn))
        ...
        using (var cmd = new GaussDBCommand($"SELECT '{bigString2}'", conn))
        using (var reader = await cmd.ExecuteReaderAsync())
        {
            reader.Read();
    ...
    [Test]
    [Description("Test whether the internal GaussDBConnection.Open method stays on the same thread with async=false")]
    ...
    public async Task PhysicalConnectionInitializer_async_with_break()
    {
        ...
        //Assert.That(async () => await dataSource.OpenConnectionAsync(), Throws.Exception.InstanceOf<GaussDBException>());
        Assert.That(dataSource.Statistics, Is.EqualTo((0, 0, 0)));
    }
    ...
    public async Task PhysicalConnectionInitializer_async_throws_on_second_open()
    {
        ...
        // With multiplexing a physical connection might open on GaussDBConnection.OpenAsync (if there was no completed bootstrap beforehand)
        // or on GaussDBCommand.ExecuteReaderAsync.
    ...
    {
        GaussDBConnection? initializerConnection = null;
    ...
    public async Task Fail_connect_with_none_auth()
    {
        ...
        var ex = Assert.ThrowsAsync<GaussDBException>(async () => await dataSource.OpenConnectionAsync())!;
    ...
    public async Task Breaking_connection_while_loading_database_info()
    {
        ...
        GaussDBDatabaseInfo.RegisterFactory(new BreakingDatabaseInfoFactory());
        try
        {
        ...
        {
            GaussDBDatabaseInfo.ResetFactories();
        }
    ...
    class BreakingDatabaseInfoFactory : IGaussDBDatabaseInfoFactory
    {
    ...
    public async Task Log_Open_Close_pooled()
    {
        ...
        var openingConnectionEvent = listLoggerProvider.Log.Single(l => l.Id == GaussDBEventId.OpeningConnection);
        ...
        var openedConnectionEvent = listLoggerProvider.Log.Single(l => l.Id == GaussDBEventId.OpenedConnection);
        ...
        var closingConnectionEvent = listLoggerProvider.Log.Single(l => l.Id == GaussDBEventId.ClosingConnection);
        ...
        var closedConnectionEvent = listLoggerProvider.Log.Single(l => l.Id == GaussDBEventId.ClosedConnection);
        ...
        var ids = new[]
        {
            GaussDBEventId.OpeningPhysicalConnection,
            GaussDBEventId.OpenedPhysicalConnection,
            GaussDBEventId.ClosingPhysicalConnection,
            GaussDBEventId.ClosedPhysicalConnection
        };
    ...
    public async Task Log_Open_Close_physical()
    {
        ...
        var csb = new GaussDBConnectionStringBuilder(ConnectionString) { Pooling = false };
        ...
        var openingConnectionEvent = listLoggerProvider.Log.Single(l => l.Id == GaussDBEventId.OpeningPhysicalConnection);
        AssertLoggingConnectionString(conn, openingConnectionEvent.State);
        AssertLoggingStateContains(openingConnectionEvent, "Host", host);
        AssertLoggingStateContains(openingConnectionEvent, "Port", port);
        AssertLoggingStateContains(openingConnectionEvent, "Database", database);

        var openedConnectionEvent = listLoggerProvider.Log.Single(l => l.Id == GaussDBEventId.OpenedPhysicalConnection);
        AssertLoggingConnectionString(conn, openedConnectionEvent.State);
        AssertLoggingStateContains(openedConnectionEvent, "ConnectorId", processId);
        AssertLoggingStateContains(openingConnectionEvent, "Host", host);
        AssertLoggingStateContains(openingConnectionEvent, "Port", port);
        ...
        var closingConnectionEvent = listLoggerProvider.Log.Single(l => l.Id == GaussDBEventId.ClosingPhysicalConnection);
        AssertLoggingConnectionString(conn, closingConnectionEvent.State);
        AssertLoggingStateContains(closingConnectionEvent, "ConnectorId", processId);
        AssertLoggingStateContains(closingConnectionEvent, "Host", host);
        AssertLoggingStateContains(closingConnectionEvent, "Port", port);
        ...
        var closededConnectionEvent = listLoggerProvider.Log.Single(l => l.Id == GaussDBEventId.ClosedPhysicalConnection);
    ...
    void AssertLoggingConnectionString(GaussDBConnection connection, object? logState)
    {
