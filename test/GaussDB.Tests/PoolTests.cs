...
class PoolTests : TestBase
{
    ...
    public async Task Timeout_getting_connector_from_exhausted_pool([Values(true, false)] bool async)
    {
        ...
        await using (var conn1 = dataSource.CreateConnection())
        {
            ...
            await using var conn2 = dataSource.CreateConnection();
            var e = async
                ? Assert.ThrowsAsync<GaussDBException>(async () => await conn2.OpenAsync())!
                : Assert.Throws<GaussDBException>(() => conn2.Open())!;
    ...
    public async Task Release_waiter_on_connection_failure()
    {
        ...
        foreach (var inner in ex.InnerExceptions)
            Assert.That(inner, Is.TypeOf<GaussDBException>());
    }
    ...
    public void ClearPool_with_busy()
    {
        ...
        var conn = new GaussDBConnection(connString);
        try
        {
    ...
    [Test, Description("https://github.com/gaussdb/gaussdb/commit/45e33ecef21f75f51a625c7b919a50da3ed8e920#r28239653")]
    ...
    void AssertPoolState(GaussDBDataSource? pool, int open, int idle)
    {
        if (pool == null)
    ...
    public async Task OnePhysicalConnectionManyCommands()
    {
        ...
        await Task.WhenAll(Enumerable.Range(0, numParallelCommands)
            ...
            {
                await using var conn = await dataSource.OpenConnectionAsync();
                await using var cmd = new GaussDBCommand("SELECT " + i, conn);
    ...
    public async Task MultiplexedCommandDoesntGetExecutedOnTransactionedConnector()
    {
        ...
        await using var conn2 = await dataSource.OpenConnectionAsync();
        await using var cmd = new GaussDBCommand("SELECT 1", conn2);
        Assert.ThrowsAsync<GaussDBException>(() => cmd.ExecuteScalarAsync());
    }
