...
public class TracingTests(MultiplexingMode multiplexingMode) : MultiplexingTestBase(multiplexingMode)
{
    ...
    public async Task Basic_open([Values] bool async)
    {
        ...
        // align with upstream https://github.com/gaussdb/gaussdb/blob/4da52a03f441f23a4f5597ac106b7833ab4fbe90/test/GaussDB.Tests/TracingTests.cs#L45
        ...
        static void ValidateActivity(Activity activity, GaussDBConnection conn, bool isMultiplexing)
        {
    ...
    async Task<object?> ExecuteScalar(GaussDBConnection connection, bool async, bool isBatch, string query)
    {
        if (!isBatch)
        {
            if (async)
