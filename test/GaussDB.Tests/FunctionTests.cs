...
public class FunctionTests : TestBase
{
    ...
    public async Task Resultset()
    {
        ...
        await using var cmd = new GaussDBCommand(function, conn) { CommandType = CommandType.StoredProcedure };
    ...
    public async Task Param_Input()
    {
        ...
        await using var cmd = new GaussDBCommand(function, conn);
...
$$ LANGUAGE plpgsql");
        ...
        var outParam = new GaussDBParameter("param_out", DbType.String) { Direction = ParameterDirection.Output };
...
$$ LANGUAGE plpgsql");
        await using var cmd = new GaussDBCommand(function, conn);
        cmd.CommandType = CommandType.StoredProcedure;
        var outParam = new GaussDBParameter("param", DbType.Int32)
        {
    ...
    public async Task Void()
    {
        ...
        var command = new GaussDBCommand("pg_sleep", conn);
    ...
    public async Task Named_parameters()
    {
        ...
        await using var command = new GaussDBCommand("make_timestamp", conn);
        command.CommandType = CommandType.StoredProcedure;
    ...
    {
        await using var conn = await OpenConnectionAsync();
        var command = new GaussDBCommand("VALUES (4,5), (6,7)", conn);
        command.Parameters.Add(new GaussDBParameter("a", DbType.Int32)
        {
            Direction = ParameterDirection.Output,
            Value = -1
        });
        command.Parameters.Add(new GaussDBParameter("b", DbType.Int32)
        {
            Direction = ParameterDirection.Output,
            Value = -1
        });
        command.Parameters.Add(new GaussDBParameter("c", DbType.Int32)
        {
            Direction = ParameterDirection.Output,
            Value = -1
        });
    ...
    public async Task ReturnValue_parameter_ignored()
    {
        ...
        await using var cmd = new GaussDBCommand(funcName, conn) { CommandType = CommandType.StoredProcedure };
    ...
    public async Task CommandBehavior_SchemaOnly_support_function_call()
    {
        ...
        var command = new GaussDBCommand(function, conn) { CommandType = CommandType.StoredProcedure };
...
$$ LANGUAGE plpgsql");
        await using var cmd = new GaussDBCommand(function, conn);
        cmd.CommandType = CommandType.StoredProcedure;
        cmd.Parameters.Add(new GaussDBParameter("param_in", DbType.Int32)
        {
            Direction = ParameterDirection.InputOutput,
            Value = 1
        });
        cmd.Parameters.Add(new GaussDBParameter("param_out", GaussDBDbType.Interval)
        {
            Direction = ParameterDirection.Output
        });
...
$$ LANGUAGE plpgsql");
        ...
        Assert.That(cmd.Parameters[1].GaussDBDbType, Is.EqualTo(GaussDBDbType.Text));
        ...
        Assert.That(cmd.Parameters[2].GaussDBDbType, Is.EqualTo(GaussDBDbType.Integer));
    ...
    public async Task DeriveParameters_function_with_case_sensitive_name()
    {
        ...
        try
        {
            await using var command = new GaussDBCommand(@"""FunctionCaseSensitive""", conn) { CommandType = CommandType.StoredProcedure };
            GaussDBCommandBuilder.DeriveParameters(command);
            Assert.AreEqual(GaussDBDbType.Integer, command.Parameters[0].GaussDBDbType);
            Assert.AreEqual(GaussDBDbType.Text, command.Parameters[1].GaussDBDbType);
        }
        finally
        {
    ...
    public async Task DeriveParameters_quote_characters_in_function_name()
    {
        ...
        try
        {
            await using var command = new GaussDBCommand(function, conn) { CommandType = CommandType.StoredProcedure };
            GaussDBCommandBuilder.DeriveParameters(command);
            Assert.AreEqual(GaussDBDbType.Integer, command.Parameters[0].GaussDBDbType);
            Assert.AreEqual(GaussDBDbType.Text, command.Parameters[1].GaussDBDbType);
        }
    ...
    public async Task DeriveParameters_dot_character_in_function_name()
    {
        ...
        try
        {
            await using var command = new GaussDBCommand(@"""My.Dotted.Function""", conn) { CommandType = CommandType.StoredProcedure };
            GaussDBCommandBuilder.DeriveParameters(command);
            Assert.AreEqual(GaussDBDbType.Integer, command.Parameters[0].GaussDBDbType);
            Assert.AreEqual(GaussDBDbType.Text, command.Parameters[1].GaussDBDbType);
        }
    ...
    {
        await using var conn = await OpenConnectionAsync();
        var invalidCommandName = new GaussDBCommand("invalidfunctionname", conn) { CommandType = CommandType.StoredProcedure };
        Assert.That(() => GaussDBCommandBuilder.DeriveParameters(invalidCommandName),
...
SET search_path TO {schema2};");
        await using var command = new GaussDBCommand("redundantfunc", conn) { CommandType = CommandType.StoredProcedure };
        GaussDBCommandBuilder.DeriveParameters(command);
...
RESET search_path;");
        await using var command = new GaussDBCommand("schema1func", conn) { CommandType = CommandType.StoredProcedure };
        Assert.That(() => GaussDBCommandBuilder.DeriveParameters(command),
            Throws.Exception.TypeOf<PostgresException>()
                .With.Property(nameof(PostgresException.SqlState)).EqualTo(PostgresErrorCodes.UndefinedFunction));
    }
...
SET search_path TO {schema1}, {schema2};");
        var command = new GaussDBCommand("redundantfunc", conn) { CommandType = CommandType.StoredProcedure };
        Assert.That(() => GaussDBCommandBuilder.DeriveParameters(command),
            Throws.Exception.TypeOf<PostgresException>()
...
#if DEBUG
    [OneTimeSetUp]
    public void OneTimeSetup() => GaussDBCommand.EnableStoredProcedureCompatMode = true;

    [OneTimeTearDown]
    public void OneTimeTeardown() => GaussDBCommand.EnableStoredProcedureCompatMode = false;
#else
    [OneTimeSetUp]
