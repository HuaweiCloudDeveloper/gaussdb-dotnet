...
public abstract class GaussDBDataSource : DbDataSource
{
    ...
    /// <summary>
    /// Contains the connection string returned to the user from <see cref="GaussDBConnection.ConnectionString"/>
    /// after the connection has been opened. Does not contain the password unless Persist Security Info=true.
    /// </summary>
    internal GaussDBConnectionStringBuilder Settings { get; }

    internal GaussDBDataSourceConfiguration Configuration { get; }
    ...
    /// </summary>
    internal GaussDBDatabaseInfo DatabaseInfo { get; private set; } = null!; // Initialized at bootstrapping
    ...
    readonly Func<GaussDBConnectionStringBuilder, string>? _passwordProvider;
    readonly Func<GaussDBConnectionStringBuilder, CancellationToken, ValueTask<string>>? _passwordProviderAsync;
    readonly Func<GaussDBConnectionStringBuilder, CancellationToken, ValueTask<string>>? _periodicPasswordProvider;
    ...
    internal Action<GaussDBConnection>? ConnectionInitializer { get; }
    internal Func<GaussDBConnection, Task>? ConnectionInitializerAsync { get; }
    ...
    private protected readonly Dictionary<Transaction, List<GaussDBConnector>> _pendingEnlistedConnectors
        = new();
    ...
    readonly IGaussDBNameTranslator _defaultNameTranslator;

    internal GaussDBDataSource(
        GaussDBConnectionStringBuilder settings,
        GaussDBDataSourceConfiguration dataSourceConfig)
    {
        Settings = settings;
    ...
    public new GaussDBConnection CreateConnection()
        => GaussDBConnection.FromDataSource(this);

    /// <inheritdoc cref="DbDataSource.OpenConnection" />
    public new GaussDBConnection OpenConnection()
    {
    ...
    public new async ValueTask<GaussDBConnection> OpenConnectionAsync(CancellationToken cancellationToken = default)
    {
        var connection = CreateConnection();
    ...
    /// <summary>
    /// Creates a command ready for use against this <see cref="GaussDBDataSource" />.
    /// </summary>
    /// <param name="commandText">An optional SQL for the command.</param>
    public new GaussDBCommand CreateCommand(string? commandText = null)
        => new GaussDBDataSourceCommand(CreateConnection()) { CommandText = commandText };

    /// <summary>
    /// Creates a batch ready for use against this <see cref="GaussDBDataSource" />.
    /// </summary>
    public new GaussDBBatch CreateBatch()
        => new GaussDBDataSourceBatch(CreateConnection());
    ...
    /// <summary>
    /// Creates a new <see cref="GaussDBDataSource" /> for the given <paramref name="connectionString" />.
    /// </summary>
    public static GaussDBDataSource Create(string connectionString)
        => new GaussDBDataSourceBuilder(connectionString).Build();

    /// <summary>
    /// Creates a new <see cref="GaussDBDataSource" /> for the given <paramref name="connectionStringBuilder" />.
    /// </summary>
    public static GaussDBDataSource Create(GaussDBConnectionStringBuilder connectionStringBuilder)
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            GaussDBDatabaseInfo databaseInfo;

            using (connector.StartUserAction(ConnectorState.Executing, cancellationToken))
                databaseInfo = await GaussDBDatabaseInfo.Load(connector, timeout, async).ConfigureAwait(false);
    ...
    /// Consider using <see cref="GaussDBDataSourceBuilder.UsePeriodicPasswordProvider" /> instead.
    /// </summary>
    ...
    internal ValueTask<string?> GetPassword(bool async, CancellationToken cancellationToken = default)
    {
        ...
        async ValueTask<string?> GetPassword(bool async, CancellationToken cancellationToken)
        {
            ...
            catch (Exception e)
            {
                ...
                throw new GaussDBException("An exception was thrown from the password provider", e);
            }
        }
    }
    ...
    async Task RefreshPassword()
    {
        ...
        catch (Exception e)
        {
            ...
            throw new GaussDBException("An exception was thrown from the periodic password provider", e);
        }
    }

    #endregion Password management

    internal abstract ValueTask<GaussDBConnector> Get(
        GaussDBConnection conn, GaussDBTimeout timeout, bool async, CancellationToken cancellationToken);

    internal abstract bool TryGetIdleConnector([NotNullWhen(true)] out GaussDBConnector? connector);

    internal abstract ValueTask<GaussDBConnector?> OpenNewConnector(
        GaussDBConnection conn, GaussDBTimeout timeout, bool async, CancellationToken cancellationToken);

    internal abstract void Return(GaussDBConnector connector);
    ...
    {
        Debug.Assert(this is not GaussDBMultiHostDataSource);
        ...
        bool ignoreTimeStamp = false)
    {
        ...
        _databaseStateInfo = new(newState, new GaussDBTimeout(stateExpiration), timeStamp);
    ...
    internal virtual void AddPendingEnlistedConnector(GaussDBConnector connector, Transaction transaction)
    {
        lock (_pendingEnlistedConnectors)
        {
            if (!_pendingEnlistedConnectors.TryGetValue(transaction, out var list))
                list = _pendingEnlistedConnectors[transaction] = new List<GaussDBConnector>(1);
            list.Add(connector);
        }
    }

    internal virtual bool TryRemovePendingEnlistedConnector(GaussDBConnector connector, Transaction transaction)
    {
        lock (_pendingEnlistedConnectors)
        {
            if (!_pendingEnlistedConnectors.TryGetValue(transaction, out var list))
    ...
    internal virtual bool TryRentEnlistedPending(Transaction transaction, GaussDBConnection connection,
    ...
    sealed class DatabaseStateInfo(DatabaseState state, GaussDBTimeout timeout, DateTime timeStamp)
    {
        internal readonly DatabaseState State = state;
        internal readonly GaussDBTimeout Timeout = timeout;
