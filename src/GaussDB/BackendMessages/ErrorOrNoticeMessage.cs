...
sealed class ErrorOrNoticeMessage
{
    ...
    internal static ErrorOrNoticeMessage Load(GaussDBReadBuffer buf, bool includeDetail, ILogger exceptionLogger)
    {
        ...
        while (true)
        {
            ...
            case ErrorFieldTypeCode.Detail:
                ...
                if (!includeDetail && !string.IsNullOrEmpty(detail))
                    detail = $"Detail redacted as it may contain sensitive data. Specify '{GaussDBConnectionStringBuilder.IncludeExceptionDetailDisplayName}' in the connection string to include this information.";
                break;
        ...
        End:
        if (severity == null)
            throw new GaussDBException("Severity not received in server error message");
        if (code == null)
            throw new GaussDBException("Code not received in server error message");
        if (message == null)
            throw new GaussDBException("Message not received in server error message");
