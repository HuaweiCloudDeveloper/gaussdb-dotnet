...
/// a PostgreSQL table. Initiated by <see cref="GaussDBConnection.BeginBinaryImport(string)"/>
/// </summary>
/// <remarks>
/// See https://www.postgresql.org/docs/current/static/sql-copy.html.
/// </remarks>
public sealed class GaussDBBinaryImporter : ICancelable
{
    #region Fields and Properties

    GaussDBConnector _connector;
    GaussDBWriteBuffer _buf;
    ...
    internal GaussDBBinaryImporter(GaussDBConnector connector)
    {
    ...
    internal async Task Init(string copyFromCommand, bool async, CancellationToken cancellationToken = default)
    {
        ...
        _params = new GaussDBParameter[copyInResponse.NumColumns];
        _rowsImported = 0;
        _buf.StartCopyMode();
        WriteHeader();
    ...
    void WriteHeader()
    {
        _buf.WriteBytes(GaussDBRawCopyStream.BinarySignature, 0, GaussDBRawCopyStream.BinarySignature.Length);
    ...
    /// corruption will occur. If in doubt, use <see cref="Write{T}(T, GaussDBDbType)"/> to manually
    /// specify the type.
    /// </typeparam>
    public void Write<T>(T value)
        => Write(async: false, value, gaussdbDbType: null, dataTypeName: null).GetAwaiter().GetResult();
    ...
    public Task WriteAsync<T>(T value, CancellationToken cancellationToken = default)
        => Write(async: true, value, gaussdbDbType: null, dataTypeName: null, cancellationToken);

    /// <summary>
    /// Writes a single column in the current row as type <paramref name="gaussdbDbType"/>.
    /// </summary>
    /// <param name="value">The value to be written</param>
    /// <param name="gaussdbDbType">
    ...
    /// <paramref name="gaussdbDbType"/> must be specified as <see cref="GaussDBDbType.Jsonb"/>.
    /// </param>
    /// <typeparam name="T">The .NET type of the column to be written.</typeparam>
    public void Write<T>(T value, GaussDBDbType gaussdbDbType) =>
        Write(async: false, value, gaussdbDbType, dataTypeName: null).GetAwaiter().GetResult();
    ...
    /// </param>
    /// <typeparam name="T">The .NET type of the column to be written.</typeparam>
    public Task WriteAsync<T>(T value, GaussDBDbType gaussdbDbType, CancellationToken cancellationToken = default)
        => Write(async: true, value, gaussdbDbType, dataTypeName: null, cancellationToken);
    ...
    public void Write<T>(T value, string dataTypeName) =>
        Write(async: false, value, gaussdbDbType: null, dataTypeName).GetAwaiter().GetResult();
    ...
    public Task WriteAsync<T>(T value, string dataTypeName, CancellationToken cancellationToken = default)
        => Write(async: true, value, gaussdbDbType: null, dataTypeName, cancellationToken);

    Task Write<T>(bool async, T value, GaussDBDbType? gaussdbDbType, string? dataTypeName, CancellationToken cancellationToken = default)
    {
        // Handle DBNull:
        ...
        return Core(async, value, gaussdbDbType, dataTypeName, cancellationToken);

        async Task Core(bool async, T value, GaussDBDbType? gaussdbDbType, string? dataTypeName, CancellationToken cancellationToken = default)
        {
            CheckReady();
            cancellationToken.ThrowIfCancellationRequested();
            CheckColumnIndex();
            ...
            var newParam = false;
            if (_params[_column] is not GaussDBParameter<T> param)
            {
                newParam = true;
                param = new GaussDBParameter<T>();
                if (gaussdbDbType is not null)
                    param._gaussdbDbType = gaussdbDbType;
                if (dataTypeName is not null)
                    param._dataTypeName = dataTypeName;
            }

            // We only retrieve previous values if anything actually changed.
            // For object typed parameters we must do so whenever setting GaussDBParameter.Value would reset the type info.
            ...
            if (previousTypeInfo is not null && previousConverter is not null && param.PgTypeId != previousTypeId)
            {
                ...
                // Also always rolling back could set PgTypeInfos that were resolved for a type that doesn't match the T of the GaussDBParameter.
                if (!newParam)
                ...
                throw new InvalidOperationException($"Write for column {_column} resolves to a different PostgreSQL type: {currentPgTypeId} than the first row resolved to ({previousTypeId}). " +
                                                    ...
                                                    $"Alternatively pass the same GaussDBDbType or DataTypeName to ensure the PostgreSQL type ends up to be identical." );
            }
    ...
    async Task WriteRow(bool async, CancellationToken cancellationToken = default, params object?[] values)
    {
        ...
        foreach (var value in values)
            await Write(async, value, gaussdbDbType: null, dataTypeName: null, cancellationToken).ConfigureAwait(false);
    }
    ...
    async Task Cancel(bool async, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            throw _connector.Break(
                new GaussDBException("Expected ErrorResponse when cancelling COPY but got: " + msg.Code));
        }
...
#pragma warning restore CS8625
    ...
    void CheckReady()
    {
        ...
        static void Throw(ImporterState state)
            => throw (state switch
            {
                ImporterState.Disposed => new ObjectDisposedException(typeof(GaussDBBinaryImporter).FullName,
