...
/// the <see cref="GaussDBConnection"/> class.
/// </summary>
...
[UnconditionalSuppressMessage("ReflectionAnalysis", "IL2113:ReflectionToRequiresUnreferencedCode",
    Justification = "Suppressing the same warnings as suppressed in the base DbConnectionStringBuilder. See https://github.com/dotnet/runtime/issues/97057")]
public sealed partial class GaussDBConnectionStringBuilder : DbConnectionStringBuilder, IDictionary<string, object?>
{
    #region Fields

    /// <summary>
    /// Cached DataSource value to reduce allocations on GaussDBConnection.DataSource.get
    /// </summary>
    ...
    /// <summary>
    /// Initializes a new instance of the GaussDBConnectionStringBuilder class.
    /// </summary>
    public GaussDBConnectionStringBuilder() => Init();

    /// <summary>
    /// Initializes a new instance of the GaussDBConnectionStringBuilder class, optionally using ODBC rules for quoting values.
    /// </summary>
    /// <param name="useOdbcRules">true to use {} to delimit fields; false to use quotation marks.</param>
    public GaussDBConnectionStringBuilder(bool useOdbcRules) : base(useOdbcRules) => Init();

    /// <summary>
    /// Initializes a new instance of the GaussDBConnectionStringBuilder class and sets its <see cref="DbConnectionStringBuilder.ConnectionString"/>.
    /// </summary>
    public GaussDBConnectionStringBuilder(string? connectionString)
    {
        Init();
    ...
    /// An idiomatic approach where each action has its own method would double the binary size of GaussDBConnectionStringBuilder.
    /// </summary>
    ...
    /// <summary>
    /// Adds an item to the <see cref="GaussDBConnectionStringBuilder"/>.
    /// </summary>
    ...
    /// <summary>
    /// Clears the contents of the <see cref="GaussDBConnectionStringBuilder"/> instance.
    /// </summary>
    ...
    /// <summary>
    /// Determines whether the <see cref="GaussDBConnectionStringBuilder"/> contains a specific key.
    /// </summary>
    /// <param name="keyword">The key to locate in the <see cref="GaussDBConnectionStringBuilder"/>.</param>
    /// <returns><b>true</b> if the <see cref="GaussDBConnectionStringBuilder"/> contains an entry with the specified key; otherwise <b>false</b>.</returns>
    ...
    /// <summary>
    /// Determines whether the <see cref="GaussDBConnectionStringBuilder"/> contains a specific key-value pair.
    /// </summary>
    /// <param name="item">The item to locate in the <see cref="GaussDBConnectionStringBuilder"/>.</param>
    /// <returns><b>true</b> if the <see cref="GaussDBConnectionStringBuilder"/> contains the entry; otherwise <b>false</b>.</returns>
    public bool Contains(KeyValuePair<string, object?> item)
        => TryGetValue(item.Key, out var value) &&
           ((value == null && item.Value == null) || (value != null && value.Equals(item.Value)));

    /// <summary>
    /// Retrieves a value corresponding to the supplied key from this <see cref="GaussDBConnectionStringBuilder"/>.
    /// </summary>
    ...
    [DisplayName("Host")]
    [GaussDBConnectionStringProperty("Server")]
    public string? Host
    {
        get => _host;
        set
    ...
    [DefaultValue(GaussDBConnection.DefaultPort)]
    public int Port
    {
        get => _port;
        set
    ...
    [GaussDBConnectionStringProperty("DB")]
    ...
    [GaussDBConnectionStringProperty("User Name", "UserId", "User Id", "UID")]
    ...
    [GaussDBConnectionStringProperty("PSW", "PWD")]
    ...
    [GaussDBConnectionStringProperty("Krbsrvname")]
    ...
    [GaussDBConnectionStringProperty("Load Balance Timeout")]
    [DefaultValue(3600)]
    ...
    public int Timeout
    {
        ...
        {
            ArgumentOutOfRangeException.ThrowIfNegative(value);
            ArgumentOutOfRangeException.ThrowIfGreaterThan(value, GaussDBConnection.TimeoutLimit);
    ...
    [GaussDBConnectionStringProperty]
    [DefaultValue(GaussDBCommand.DefaultTimeout)]
    ...
    /// </summary>
    [Category("Timeouts")]
    [Description("After Command Timeout is reached (or user supplied cancellation token is cancelled) and command cancellation is attempted, GaussDB waits for this additional timeout (in milliseconds) before breaking the connection. Defaults to 2000, set to zero for infinity.")]
    ...
    // 只有显式启用 AZ 分簇或簇内 CN 重排时，才走增强 HA 路由分支。
    ...
    /// <summary>
    /// The number of seconds of connection inactivity before GaussDB sends a keepalive query.
    /// Set to 0 (the default) to disable.
    /// </summary>
    [Category("Advanced")]
    [Description("The number of seconds of connection inactivity before GaussDB sends a keepalive query.")]
    ...
    /// <summary>
    /// Determines the size of the internal buffer GaussDB uses when reading. Increasing may improve performance if transferring large values from the database.
    /// </summary>
    [Category("Advanced")]
    [Description("Determines the size of the internal buffer GaussDB uses when reading. Increasing may improve performance if transferring large values from the database.")]
    [DisplayName("Read Buffer Size")]
    [GaussDBConnectionStringProperty]
    [DefaultValue(GaussDBReadBuffer.DefaultSize)]
    ...
    /// <summary>
    /// Determines the size of the internal buffer GaussDB uses when writing. Increasing may improve performance if transferring large values to the database.
    /// </summary>
    [Category("Advanced")]
    [Description("Determines the size of the internal buffer GaussDB uses when writing. Increasing may improve performance if transferring large values to the database.")]
    [DisplayName("Write Buffer Size")]
    [GaussDBConnectionStringProperty]
    [DefaultValue(GaussDBWriteBuffer.DefaultSize)]
    ...
    public int InternalCommandTimeout
    {
        ...
        set
        {
            if (value != 0 && value != -1 && value < GaussDBConnector.MinimumInternalCommandTimeout)
                throw new ArgumentOutOfRangeException(nameof(value), value,
                    $"InternalCommandTimeout must be >= {GaussDBConnector.MinimumInternalCommandTimeout}, 0 (infinite) or -1 (use CommandTimeout)");
    ...
    internal void PostProcessAndValidate()
    {
        ...
        if (!Host.Contains(','))
        {
            if (TargetSessionAttributesParsed is not null &&
                TargetSessionAttributesParsed != GaussDB.TargetSessionAttributes.Any)
            {
    ...
    public override bool Equals(object? obj)
        => obj is GaussDBConnectionStringBuilder o && EquivalentTo(o);
    ...
    /// <summary>
    /// Gets an <see cref="ICollection" /> containing the keys of the <see cref="GaussDBConnectionStringBuilder"/>.
    /// </summary>
    ...
    /// <summary>
    /// Gets an <see cref="ICollection" /> containing the values in the <see cref="GaussDBConnectionStringBuilder"/>.
    /// </summary>
    ...
    /// <summary>
    /// Copies the elements of the <see cref="GaussDBConnectionStringBuilder"/> to an Array, starting at a particular Array index.
    /// </summary>
    /// <param name="array">
    /// The one-dimensional Array that is the destination of the elements copied from <see cref="GaussDBConnectionStringBuilder"/>.
    ...
    /// <summary>
    /// Returns an enumerator that iterates through the <see cref="GaussDBConnectionStringBuilder"/>.
    /// </summary>
    /// <returns></returns>
    ...
    protected override void GetProperties(Hashtable propertyDescriptors)
    {
        ...
        foreach (var value in propertyDescriptors.Values)
        {
            ...
            foreach (var attribute in d.Attributes)
                if (attribute is GaussDBConnectionStringPropertyAttribute or ObsoleteAttribute)
                    toRemove.Add(d);
        }
...
/// <summary>
/// Marks on <see cref="GaussDBConnectionStringBuilder"/> which participate in the connection
/// string. Optionally holds a set of synonyms for the property.
/// </summary>
[AttributeUsage(AttributeTargets.Property)]
sealed class GaussDBConnectionStringPropertyAttribute : Attribute
{
    /// <summary>
    ...
    /// <summary>
    /// Creates a <see cref="GaussDBConnectionStringPropertyAttribute"/>.
    /// </summary>
    public GaussDBConnectionStringPropertyAttribute()
        => Synonyms = [];

    /// <summary>
    /// Creates a <see cref="GaussDBConnectionStringPropertyAttribute"/>.
    /// </summary>
    public GaussDBConnectionStringPropertyAttribute(params string[] synonyms)
        => Synonyms = synonyms;
}
...
public enum ArrayNullabilityMode
{
    ...
    /// that the actual type of array instances returned from APIs like <see cref="GaussDBDataReader.GetValue"/>
