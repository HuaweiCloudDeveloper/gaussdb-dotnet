...
sealed class MultiplexingDataSource : PoolingDataSource
{
    ...
    readonly ChannelReader<GaussDBCommand> _multiplexCommandReader;
    internal ChannelWriter<GaussDBCommand> MultiplexCommandWriter { get; }
        ...
        : base(settings, dataSourceConfig)
    {
        ...
        var multiplexCommandChannel = Channel.CreateBounded<GaussDBCommand>(
        ...
        _multiplexWriteLoop = Task.Run(MultiplexingWriteLoop, CancellationToken.None)
            .ContinueWith(t =>
            {
                if (t.IsFaulted)
                {
                    ...
                    _connectionLogger.LogError(t.Exception, "Exception in multiplexing write loop, this is an GaussDB bug, please file an issue.");
                }
            });
    }
    ...
    async Task MultiplexingWriteLoop()
    {
                ...
                while (true)
                {
                    ...
                    try
                    {
                        connector = await OpenNewConnector(
                            ...
                            new GaussDBTimeout(TimeSpan.FromSeconds(Settings.Timeout)),
                            async: true,
        ...
        bool WriteCommand(GaussDBConnector connector, GaussDBCommand command, ref MultiplexingStats stats)
        {
            ...
            default:
                ...
                ThrowHelper.ThrowGaussDBException("When writing command to connector, task is in invalid state " + task.Status);
                return false;
            }
        }

        void Flush(GaussDBConnector connector, ref MultiplexingStats stats)
        {
            ...
            default:
                ...
                ThrowHelper.ThrowGaussDBException("When flushing, task is in invalid state " + task.Status);
                return;
            }
        }

        void FailWrite(GaussDBConnector connector, Exception exception)
        {
        ...
        static void CompleteWrite(GaussDBConnector connector, ref MultiplexingStats stats)
        {
            ...
            GaussDBEventSource.Log.MultiplexingBatchSent(stats.NumCommands, Stopwatch.GetElapsedTime(stats.StartTimestamp).Ticks);
        }
