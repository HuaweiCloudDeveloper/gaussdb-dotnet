...
public class GaussDBCommand : DbCommand, ICloneable, IComponent
{
    #region Fields

    GaussDBTransaction? _transaction;

    readonly GaussDBConnector? _connector;
    ...
    /// </summary>
    GaussDBConnector? _connectorPreparedOn;
    ...
    internal GaussDBBatch? WrappingBatch { get; }

    internal List<GaussDBBatchCommand> InternalBatchCommands { get; }
    ...
    public IReadOnlyList<GaussDBBatchCommand> Statements => InternalBatchCommands.AsReadOnly();
    ...
    /// <summary>
    /// Whether this command is cached by <see cref="GaussDBConnection" /> and returned by <see cref="GaussDBConnection.CreateCommand" />.
    /// </summary>
    ...
    static GaussDBCommand()
    {
        EnableSqlRewriting = !AppContext.TryGetSwitch("GaussDB.EnableSqlRewriting", out var enabled) || enabled;
        EnableStoredProcedureCompatMode = AppContext.TryGetSwitch("GaussDB.EnableStoredProcedureCompatMode", out enabled) && enabled;
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="GaussDBCommand"/> class.
    /// </summary>
    public GaussDBCommand() : this(null, null, null) {}

    /// <summary>
    /// Initializes a new instance of the <see cref="GaussDBCommand"/> class with the text of the query.
    /// </summary>
    /// <param name="cmdText">The text of the query.</param>
    // ReSharper disable once IntroduceOptionalParameters.Global
    public GaussDBCommand(string? cmdText) : this(cmdText, null, null) {}

    /// <summary>
    /// Initializes a new instance of the <see cref="GaussDBCommand"/> class with the text of the query and a
    /// <see cref="GaussDBConnection"/>.
    /// </summary>
    /// <param name="cmdText">The text of the query.</param>
    /// <param name="connection">A <see cref="GaussDBConnection"/> that represents the connection to a PostgreSQL server.</param>
    // ReSharper disable once IntroduceOptionalParameters.Global
    public GaussDBCommand(string? cmdText, GaussDBConnection? connection)
    {
        GC.SuppressFinalize(this);
        InternalBatchCommands = new List<GaussDBBatchCommand>(1);
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="GaussDBCommand"/> class with the text of the query, a
    /// <see cref="GaussDBConnection"/>, and the <see cref="GaussDBTransaction"/>.
    /// </summary>
    /// <param name="cmdText">The text of the query.</param>
    /// <param name="connection">A <see cref="GaussDBConnection"/> that represents the connection to a PostgreSQL server.</param>
    /// <param name="transaction">The <see cref="GaussDBTransaction"/> in which the <see cref="GaussDBCommand"/> executes.</param>
    public GaussDBCommand(string? cmdText, GaussDBConnection? connection, GaussDBTransaction? transaction)
        : this(cmdText, connection)
        => Transaction = transaction;

    /// <summary>
    /// Used when this <see cref="GaussDBCommand"/> instance is wrapped inside an <see cref="GaussDBBatch"/>.
    /// </summary>
    internal GaussDBCommand(GaussDBBatch batch, int batchCommandCapacity, GaussDBConnection? connection = null)
    {
        GC.SuppressFinalize(this);
        InternalBatchCommands = new List<GaussDBBatchCommand>(batchCommandCapacity);
        InternalConnection = connection;
        CommandType = CommandType.Text;
        WrappingBatch = batch;
    ...
    internal GaussDBCommand(string? cmdText, GaussDBConnector connector) : this(cmdText)
        => _connector = connector;

    /// <summary>
    /// Used when this <see cref="GaussDBCommand"/> instance is wrapped inside an <see cref="GaussDBBatch"/>.
    /// </summary>
    internal GaussDBCommand(GaussDBBatch batch, GaussDBConnector connector, int batchCommandCapacity)
        : this(batch, batchCommandCapacity)
        => _connector = connector;

    internal static GaussDBCommand CreateCachedCommand(GaussDBConnection connection)
    ...
    /// <summary>
    /// Gets or sets a value indicating how the <see cref="GaussDBCommand.CommandText"/> property is to be interpreted.
    /// </summary>
    /// <value>
    ...
    internal GaussDBConnection? InternalConnection { get; private set; }
    ...
    /// <summary>
    /// Gets or sets the <see cref="GaussDBConnection"/> used by this instance of the <see cref="GaussDBCommand"/>.
    /// </summary>
    ...
    {
        get => (GaussDBConnection?)DbConnection;
    ...
    /// Unknown result columns are requested from PostgreSQL in text format, and GaussDB makes no
    ...
    /// <summary>
    /// Creates a new instance of a <see cref="GaussDBParameter"/> object.
    /// </summary>
    /// <returns>An <see cref="GaussDBParameter"/> object.</returns>
    public new GaussDBParameter CreateParameter() => new();
    ...
    /// <summary>
    /// Gets the <see cref="GaussDBParameterCollection"/>.
    /// </summary>
...
";
    ...
    internal void DeriveParameters()
    {
        ...
        foreach (var s in InternalBatchCommands)
            if (s.PreparedStatement?.IsExplicit == true)
                throw new GaussDBException("Deriving parameters isn't supported for commands that are already prepared.");
    ...
    {
        using var c = new GaussDBCommand(DeriveParametersForFunctionQuery, InternalConnection);
        c.Parameters.Add(new GaussDBParameter("proname", GaussDBDbType.Text));
    ...
    void DeriveParametersForQuery(GaussDBConnector connector)
    {
            ...
            try
            {
                foreach (var batchCommand in InternalBatchCommands)
                {
                    ...
                    if (batchCommand.PositionalParameters.Count != paramTypeOIDs.Count)
                    {
                        ...
                        Parameters.Clear();
                        throw new GaussDBException(
                            "There was a mismatch in the number of derived parameters between the GaussDB SQL parser and the PostgreSQL parser. Please report this as bug to the GaussDB developers (https://github.com/HuaweiCloudDeveloper/gaussdb-dotnet/issues).");
                    }
                    ...
                    for (var i = 0; i < paramTypeOIDs.Count; i++)
                    {
                        try
                        {
                            ...
                            var gaussdbDbType = postgresType.GetRepresentationalType().DataTypeName.ToGaussDBDbType();
                            if (param.GaussDBDbType != GaussDBDbType.Unknown && param.GaussDBDbType != gaussdbDbType)
                                throw new GaussDBException(
    ...
    Task Prepare(bool async, CancellationToken cancellationToken = default)
    {
        ...
        static async Task PrepareLong(GaussDBCommand command, bool async, GaussDBConnector connector, CancellationToken cancellationToken)
        {
            try
            {
    ...
    internal void ProcessRawQuery(SqlQueryParser? parser, bool standardConformingStrings, GaussDBBatchCommand? batchCommand)
    {
        ...
        case CommandType.Text:
            ...
            case PlaceholderType.Named:
                if (!EnableSqlRewriting)
                    ThrowHelper.ThrowNotSupportedException($"Named parameters are not supported when GaussDB.{nameof(EnableSqlRewriting)} is disabled");

                // The parser is cached on GaussDBConnector - unless we're in multiplexing mode.
        ...
        case CommandType.StoredProcedure:
            ...
            var inputParameters = GaussDBBatchCommand.EmptyParameters;
            if (parameters is not null)
            {
                inputParameters = new List<GaussDBParameter>(parameters.Count);
                for (var i = 0; i < parameters.Count; i++)
                {
                    ...
                    {
                        if (seenNamedParam)
                            ThrowHelper.ThrowArgumentException(GaussDBStrings.PositionalParameterAfterNamed);
                    }
                    else
                    {
                        seenNamedParam = true;
        ...
        default:
            ThrowHelper.ThrowArgumentOutOfRangeException(nameof(CommandType), $"Internal GaussDB bug: unexpected value {{0}} of enum {nameof(CommandType)}. Please file a bug.", commandType);
            break;
        }

        static void ValidateParameterCount(GaussDBBatchCommand batchCommand)
        {
            if (batchCommand is { HasParameters: true, PositionalParameters.Count: > ushort.MaxValue })
                ThrowHelper.ThrowGaussDBException("A statement cannot have more than 65535 parameters");
        }
    }
    ...
    void BeginSend(GaussDBConnector connector)
        => connector.WriteBuffer.Timeout = TimeSpan.FromSeconds(CommandTimeout);

    internal Task Write(GaussDBConnector connector, bool async, bool flush, CancellationToken cancellationToken = default)
    {
        ...
        async Task WriteExecute(GaussDBConnector connector, bool async, bool flush, CancellationToken cancellationToken)
        {
            GaussDBBatchCommand? batchCommand = null;
        ...
        async Task WriteExecuteSchemaOnly(GaussDBConnector connector, bool async, bool flush, CancellationToken cancellationToken)
        {
    ...
    async Task SendDeriveParameters(GaussDBConnector connector, bool async, CancellationToken cancellationToken = default)
    {
        BeginSend(connector);
        ...
        for (var i = 0; i < InternalBatchCommands.Count; i++)
        {
            ...
            await connector.WriteParse(batchCommand.FinalCommandText!, [], GaussDBBatchCommand.EmptyParameters, async, cancellationToken).ConfigureAwait(false);
    ...
    async Task SendPrepare(GaussDBConnector connector, bool async, CancellationToken cancellationToken = default)
    {
        BeginSend(connector);
    ...
    async Task SendClose(GaussDBConnector connector, bool async, CancellationToken cancellationToken = default)
    {
        BeginSend(connector);
    ...
    /// and returns a <see cref="GaussDBDataReader"/>.
    /// </summary>
    ...
    /// </param>
    /// <returns>A task representing the asynchronous operation.</returns>
    public new Task<GaussDBDataReader> ExecuteReaderAsync(CommandBehavior behavior, CancellationToken cancellationToken = default)
        => ExecuteReader(async: true, behavior, cancellationToken).AsTask();

    // TODO: Maybe pool these?
    internal ManualResetValueTaskSource<GaussDBConnector> ExecutionCompletion { get; }
        = new();

    internal virtual async ValueTask<GaussDBDataReader> ExecuteReader(bool async, CommandBehavior behavior, CancellationToken cancellationToken)
    {
        var conn = CheckAndGetConnection();
        _behavior = behavior;
        ...
        if (_connector is not null)
        {
            ...
            if (behavior.HasFlag(CommandBehavior.CloseConnection))
                ThrowHelper.ThrowArgumentException($"{nameof(CommandBehavior.CloseConnection)} is not supported with {nameof(GaussDBConnector)}", nameof(behavior));
        ...
        try
        {
            if (connector is not null)
            {
                ...
                try
                {
                    ...
                    case true:
                        ...
                        GaussDBEventSource.Log.CommandStartPrepared();
                    ...
                    GaussDBEventSource.Log.CommandStart(CommandText);
            ...
            else
            {
                ...
                var dataSource = (MultiplexingDataSource)conn.GaussDBDataSource;
                ...
                TraceCommandStart(conn.Settings, conn.GaussDBDataSource.Configuration.TracingOptions);
                ...
                connector = await new ValueTask<GaussDBConnector>(ExecutionCompletion, ExecutionCompletion.Version).ConfigureAwait(false);
        ...
        catch (Exception e)
        {
            ...
            if (e is not GaussDBOperationInProgressException && reader is not null)
    ...
    {
        get => _transaction;
        set => _transaction = (GaussDBTransaction?)value;
    }

    /// <summary>
    /// This property is ignored by GaussDB. PostgreSQL only supports a single transaction at a given time on
    /// a given connection, and all commands implicitly run inside the current transaction started via
    /// <see cref="GaussDBConnection.BeginTransaction()"/>
    /// </summary>
    public new GaussDBTransaction? Transaction
    {
        get => (GaussDBTransaction?)DbTransaction;
    ...
    /// <summary>
    /// Attempts to cancel the execution of an <see cref="GaussDBCommand" />.
    /// </summary>
    ...
    internal void Reset()
    {
        // TODO: Optimize GaussDBParameterCollection to recycle GaussDBParameter instances as well
    ...
    internal void TraceCommandStart(GaussDBConnectionStringBuilder settings, GaussDBTracingOptions tracingOptions)
    {
        Debug.Assert(CurrentActivity is null);

        if (GaussDBActivitySource.IsEnabled)
        {
            ...
            if (enableTracing)
            {
                ...
                CurrentActivity = GaussDBActivitySource.CommandStart(
                    settings,
    ...
    internal void TraceCommandEnrich(GaussDBConnector connector)
    {
        if (CurrentActivity is not null)
        {
            GaussDBActivitySource.Enrich(CurrentActivity, connector);
    ...
    internal void TraceReceivedFirstResponse(GaussDBTracingOptions tracingOptions)
    {
        if (CurrentActivity is not null)
            GaussDBActivitySource.ReceivedFirstResponse(CurrentActivity, tracingOptions);
    }
    ...
    internal void TraceCommandStop()
    {
        if (CurrentActivity is not null)
        {
            GaussDBActivitySource.CommandStop(CurrentActivity);
    ...
    internal void TraceSetException(Exception e)
    {
        if (CurrentActivity is not null)
        {
            GaussDBActivitySource.SetException(CurrentActivity, e);
            CurrentActivity = null;
        }
    }
    ...
    GaussDBBatchCommand TruncateStatementsToOne()
    {
        switch (InternalBatchCommands.Count)
        {
        case 0:
            var statement = new GaussDBBatchCommand();
    ...
    internal void LogExecutingCompleted(GaussDBConnector connector, bool executing)
    {
        ...
        static object[] GetParametersForLogging(GaussDBBatchCommand c)
        {
    ...
    /// </summary>
    /// <returns>A new GaussDBCommand object.</returns>
    ...
    /// </summary>
    /// <returns>A new GaussDBCommand object.</returns>
    public virtual GaussDBCommand Clone()
    {
        var clone = new GaussDBCommand(CommandText, InternalConnection, Transaction)
        {
    ...
    GaussDBConnection? CheckAndGetConnection()
    {
    ...
    /// <summary>
    /// This event is unsupported by GaussDB. Use <see cref="System.Data.Common.DbConnection.StateChange"/> instead.
    /// </summary>
