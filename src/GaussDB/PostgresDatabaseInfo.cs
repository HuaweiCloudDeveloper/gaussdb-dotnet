...
/// </summary>
sealed class PostgresDatabaseInfoFactory : IGaussDBDatabaseInfoFactory
{
    /// <inheritdoc />
...
/// <summary>
/// The default implementation of GaussDBDatabase, for standard PostgreSQL databases.
/// </summary>
class PostgresDatabaseInfo : GaussDBDatabaseInfo
{
    ...
    internal PostgresDatabaseInfo(GaussDBConnector conn)
    ...
    /// </returns>
    internal async Task LoadPostgresInfo(GaussDBConnector conn, GaussDBTimeout timeout, bool async)
    {
        HasIntegerDateTimes =
    ...
    /// types).
    /// </remarks>
    static string GenerateLoadTypesQuery(bool withRange, bool withMultirange, bool loadTableComposites, string? schemaListSqlFragment, bool hasTypeCategory)
        => $@"
...
ORDER BY oid{(withEnumSortOrder ? ", enumsortorder" : "")};";
    ...
    internal async Task<List<PostgresType>> LoadBackendTypes(GaussDBConnector conn, GaussDBTimeout timeout, bool async)
    {
