...
class PoolingDataSource : GaussDBDataSource
{
    ...
    /// </summary>
    private protected readonly GaussDBConnector?[] Connectors;
    ...
    /// </summary>
    readonly ChannelReader<GaussDBConnector?> _idleConnectorReader;
    internal ChannelWriter<GaussDBConnector?> IdleConnectorWriter { get; }

    readonly ILogger _logger;

    /// <summary>
    /// Incremented every time this pool is cleared via <see cref="GaussDBConnection.ClearPool"/> or
    /// <see cref="GaussDBConnection.ClearAllPools"/>. Allows us to identify connections which were
        ...
        : base(settings, dataSourceConfig)
    {
        ...
        var idleChannel = Channel.CreateUnbounded<GaussDBConnector?>();
        ...
        Connectors = new GaussDBConnector[MaxConnections];
    ...
    internal sealed override ValueTask<GaussDBConnector> Get(
        GaussDBConnection conn, GaussDBTimeout timeout, bool async, CancellationToken cancellationToken)
    {
        ...
        return TryGetIdleConnector(out var connector)
            ? new ValueTask<GaussDBConnector>(connector)
            : RentAsync(conn, timeout, async, cancellationToken);

        async ValueTask<GaussDBConnector> RentAsync(
            GaussDBConnection conn, GaussDBTimeout timeout, bool async, CancellationToken cancellationToken)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    {
                        throw new GaussDBException("The connection pool has been shut down.");
                    }
    ...
    internal sealed override bool TryGetIdleConnector([NotNullWhen(true)] out GaussDBConnector? connector)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    bool CheckIdleConnector([NotNullWhen(true)] GaussDBConnector? connector)
    {
    ...
    internal sealed override async ValueTask<GaussDBConnector?> OpenNewConnector(
        GaussDBConnection conn, GaussDBTimeout timeout, bool async, CancellationToken cancellationToken)
    {
        ...
        for (var numConnectors = _numConnectors; numConnectors < MaxConnections; numConnectors = _numConnectors)
        {
            ...
            try
            {
                ...
                var connector = new GaussDBConnector(this, conn) { ClearCounter = _clearCounter };
                ...
                if (i == MaxConnections)
                    throw new GaussDBException($"Could not find free slot in {Connectors} when opening. Please report a bug.");
                ...
                // on GaussDBConnector.Open due to timeout, CancellationToken or other reasons.
    ...
    internal sealed override void Return(GaussDBConnector connector)
    {
    ...
    void CloseConnector(GaussDBConnector connector)
    {
        try
        {
            connector.Close();
