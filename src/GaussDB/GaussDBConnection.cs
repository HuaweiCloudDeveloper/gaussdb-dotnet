...
public sealed class GaussDBConnection : DbConnection, ICloneable, IComponent
{
    #region Fields
    ...
    /// </summary>
    internal GaussDBConnector? Connector { get; set; }

    /// <summary>
    /// The parsed connection string. Set only after the connection is opened.
    /// </summary>
    internal GaussDBConnectionStringBuilder Settings { get; private set; } = DefaultSettings;

    static readonly GaussDBConnectionStringBuilder DefaultSettings = new();

    GaussDBDataSource? _dataSource;

    internal GaussDBDataSource GaussDBDataSource
    {
        get
        {
    ...
    public static IGaussDBTypeMapper GlobalTypeMapper => TypeMapping.GlobalTypeMapper.Instance;

    /// <summary>
    /// Connection-level type mapping is no longer supported. See the 7.0 release notes for configuring type mapping on GaussDBDataSource.
    /// </summary>
    [Obsolete("Connection-level type mapping is no longer supported. See the 7.0 release notes for configuring type mapping on GaussDBDataSource.", true)]
    public IGaussDBTypeMapper TypeMapper
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="GaussDBConnection"/> class.
    /// </summary>
    public GaussDBConnection()
        => GC.SuppressFinalize(this);

    /// <summary>
    /// Initializes a new instance of <see cref="GaussDBConnection"/> with the given connection string.
    /// </summary>
    /// <param name="connectionString">The connection used to open the PostgreSQL database.</param>

    public GaussDBConnection(string? connectionString) : this()
        => ConnectionString = connectionString;

    internal GaussDBConnection(GaussDBDataSource dataSource, GaussDBConnector connector) : this()
    {
    ...
    internal static GaussDBConnection FromDataSource(GaussDBDataSource dataSource)
        => new()
        {
            _dataSource = dataSource,
            Settings = dataSource.Settings,
    ...
    /// </summary>
    /// <remarks>
    /// Do not invoke other methods and properties of the <see cref="GaussDBConnection"/> object until the returned Task is complete.
    /// </remarks>
    ...
    void SetupDataSource()
    {
        ...
        var settings = new GaussDBConnectionStringBuilder(_connectionString);
        ...
        // ordering). Have GaussDBConnectionStringBuilder produce a canonical string representation
        // and recheck.
        ...
        if (PoolManager.Pools.TryGetValue(canonical, out _dataSource))
        {
            ...
            if (_dataSource is GaussDBMultiHostDataSource multiHostDataSource && settings.TargetSessionAttributesParsed.HasValue)
        ...
        var dataSourceBuilder = new GaussDBDataSourceBuilder(canonical);
        dataSourceBuilder.UseLoggerFactory(GaussDBLoggingConfiguration.GlobalLoggerFactory);
        dataSourceBuilder.EnableParameterLogging(GaussDBLoggingConfiguration.GlobalIsParameterLoggingEnabled);
        var newDataSource = dataSourceBuilder.Build();

        // See Clone() on the following line:
        _cloningInstantiator = s => new GaussDBConnection(s);
        ...
        if (_dataSource == newDataSource)
        {
            ...
            if (_dataSource is GaussDBMultiHostDataSource multiHostConnectorPool)
                foreach (var hostPool in multiHostConnectorPool.Pools)
                    GaussDBEventSource.Log.DataSourceCreated(hostPool);
            else
            {
                GaussDBEventSource.Log.DataSourceCreated(newDataSource);
            }
        }
        else
        ...
        // If this is a multi-host data source and the user specified a TargetSessionAttributes, create a wrapper in front of the
        // MultiHostDataSource with that TargetSessionAttributes.
        if (_dataSource is GaussDBMultiHostDataSource multiHostDataSource2 && settings.TargetSessionAttributesParsed.HasValue)
    ...
    internal Task Open(bool async, CancellationToken cancellationToken)
    {
        ...
        async Task OpenAsync(bool async, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var timeout = new GaussDBTimeout(connectionTimeout);
        ...
        async Task PerformMultiplexingStartupCheck(bool async, CancellationToken cancellationToken)
        {
            try
            {
                var timeout = new GaussDBTimeout(TimeSpan.FromSeconds(ConnectionTimeout));
    ...
    /// </p>
    /// <p>
    /// The <see cref="GaussDBConnectionStringBuilder.Password"/> and <see cref="GaussDBConnectionStringBuilder.Passfile"/> connection
    ...
    /// </p>
    /// </remarks>
    [Obsolete("Use GaussDBDataSourceBuilder.UsePeriodicPasswordProvider or inject passwords directly into GaussDBDataSource.Password")]
    ...
    public ConnectionState FullState
    {
        ...
        get
        {
            ...
            case ConnectorState.Closed:
                ThrowHelper.ThrowInvalidOperationException("Internal GaussDB bug: connection is in state Open but connector is in state Closed");
                return ConnectionState.Broken;
            default:
                ThrowHelper.ThrowInvalidOperationException($"Internal GaussDB bug: unexpected value {{0}} of enum {nameof(ConnectorState)}. Please file a bug.", Connector.State);
                return ConnectionState.Broken;
            }
        }
        internal set
    ...
    /// </summary>
    internal GaussDBCommand? CachedCommand { get; set; }
    ...
    /// <summary>
    /// Creates and returns a <see cref="GaussDBCommand"/> object associated with the <see cref="GaussDBConnection"/>.
    /// </summary>
    /// <returns>A <see cref="GaussDBCommand"/> object.</returns>
    public new GaussDBCommand CreateCommand()
    {
        CheckDisposed();
    ...
    public new GaussDBBatch CreateBatch()
    {
        CheckDisposed();
        ...
        return GaussDBBatch.CreateCachedBatch(this);
    }
    ...
    /// </summary>
    /// <returns>A <see cref="GaussDBTransaction"/> object representing the new transaction.</returns>
    /// <remarks>
    /// Nested transactions are not supported.
    /// Transactions created by this method will have the <see cref="IsolationLevel.ReadCommitted"/> isolation level.
    /// </remarks>
    public new GaussDBTransaction BeginTransaction()
    ...
    /// <returns>A <see cref="GaussDBTransaction"/> object representing the new transaction.</returns>
    /// <remarks>Nested transactions are not supported.</remarks>
    public new GaussDBTransaction BeginTransaction(IsolationLevel level)
        => BeginTransaction(async: false, level, CancellationToken.None).GetAwaiter().GetResult();

    async ValueTask<GaussDBTransaction> BeginTransaction(bool async, IsolationLevel level, CancellationToken cancellationToken)
    {
        ...
        var connector = ConnectorBindingScope == ConnectorBindingScope.Transaction
            ? Connector
            : await StartBindingScope(ConnectorBindingScope.Transaction, GaussDBTimeout.Infinite, async, cancellationToken).ConfigureAwait(false);
        ...
        try
        {
            ...
            connector.Transaction ??= new GaussDBTransaction(connector);
    ...
    /// Transactions created by this method will have the <see cref="IsolationLevel.ReadCommitted"/> isolation level.
    /// </remarks>
    public new ValueTask<GaussDBTransaction> BeginTransactionAsync(CancellationToken cancellationToken = default)
    ...
    /// Nested transactions are not supported.
    /// </remarks>
    public new ValueTask<GaussDBTransaction> BeginTransactionAsync(IsolationLevel level, CancellationToken cancellationToken = default)
    ...
    {
        // Even though GaussDBConnection isn't thread safe we'll make sure this part is.
    ...
    async Task CloseAsync(bool async)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    // We're already doing the same in the GaussDBConnector.Reset for pooled connections
    ...
    /// <summary>
    /// Releases all resources used by the <see cref="GaussDBConnection"/>.
    /// </summary>
    ...
    internal void OnNotice(PostgresNotice e)
    {
        try
        {
            Notice?.Invoke(this, new GaussDBNoticeEventArgs(e));
        }
        catch (Exception ex)
        {
    ...
    internal void OnNotification(GaussDBNotificationEventArgs e)
    {
        try
        {
    ...
    /// <returns>A <see cref="GaussDBBinaryImporter"/> which can be used to write rows and columns</returns>
    /// <remarks>
    /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
    /// </remarks>
    public GaussDBBinaryImporter BeginBinaryImport(string copyFromCommand)
    ...
    /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
    /// </remarks>
    public Task<GaussDBBinaryImporter> BeginBinaryImportAsync(string copyFromCommand, CancellationToken cancellationToken = default)
        => BeginBinaryImport(async: true, copyFromCommand, cancellationToken);

    async Task<GaussDBBinaryImporter> BeginBinaryImport(bool async, string copyFromCommand, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var importer = new GaussDBBinaryImporter(connector);
    ...
    /// <returns>A <see cref="GaussDBBinaryExporter"/> which can be used to read rows and columns</returns>
    /// <remarks>
    /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
    /// </remarks>
    public GaussDBBinaryExporter BeginBinaryExport(string copyToCommand)
    ...
    /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
    /// </remarks>
    public Task<GaussDBBinaryExporter> BeginBinaryExportAsync(string copyToCommand, CancellationToken cancellationToken = default)
        => BeginBinaryExport(async: true, copyToCommand, cancellationToken);

    async Task<GaussDBBinaryExporter> BeginBinaryExport(bool async, string copyToCommand, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var exporter = new GaussDBBinaryExporter(connector);
    ...
    async Task<TextWriter> BeginTextImport(bool async, string copyFromCommand, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var copyStream = new GaussDBRawCopyStream(connector);
            await copyStream.Init(copyFromCommand, async, cancellationToken).ConfigureAwait(false);
            var writer = new GaussDBCopyTextWriter(connector, copyStream);
    ...
    async Task<TextReader> BeginTextExport(bool async, string copyToCommand, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var reader = new GaussDBCopyTextReader(connector, copyStream);
    ...
    /// <returns>A <see cref="GaussDBRawCopyStream"/> that can be used to read or write raw binary data.</returns>
    /// <remarks>
    /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
    /// </remarks>
    public GaussDBRawCopyStream BeginRawBinaryCopy(string copyCommand)
    ...
    /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
    /// </remarks>
    public Task<GaussDBRawCopyStream> BeginRawBinaryCopyAsync(string copyCommand, CancellationToken cancellationToken = default)
        => BeginRawBinaryCopy(async: true, copyCommand, cancellationToken);

    async Task<GaussDBRawCopyStream> BeginRawBinaryCopy(bool async, string copyCommand, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var stream = new GaussDBRawCopyStream(connector);
    ...
    /// </summary>
    internal bool TryGetBoundConnector([NotNullWhen(true)] out GaussDBConnector? connector)
    {
        if (ConnectorBindingScope == ConnectorBindingScope.None)
        {
    ...
    /// </summary>
    internal ValueTask<GaussDBConnector> StartBindingScope(
        ConnectorBindingScope scope, GaussDBTimeout timeout, bool async, CancellationToken cancellationToken)
    {
        ...
        if (ConnectorBindingScope != ConnectorBindingScope.None)
        {
            ...
            return new ValueTask<GaussDBConnector>(Connector);
        }

        return StartBindingScopeAsync();

        async ValueTask<GaussDBConnector> StartBindingScopeAsync()
        {
            try
            {
                Debug.Assert(Settings.Multiplexing);
    ...
    internal GaussDBConnector StartBindingScope(ConnectorBindingScope scope)
        => StartBindingScope(scope, GaussDBTimeout.Infinite, async: false, CancellationToken.None)
            .GetAwaiter().GetResult();

    internal EndScopeDisposable StartTemporaryBindingScope(out GaussDBConnector connector)
    {
        connector = StartBindingScope(ConnectorBindingScope.Temporary);
        return new EndScopeDisposable(this);
    }

    internal T CheckOpenAndRunInTemporaryScope<T>(Func<GaussDBConnector, T> func)
    {
        CheckOpen();
    ...
    public override DataTable GetSchema(string? collectionName, string?[]? restrictions)
        => GaussDBSchema.GetSchema(async: false, this, collectionName, restrictions).GetAwaiter().GetResult();
    ...
    public override Task<DataTable> GetSchemaAsync(string collectionName, string?[]? restrictions, CancellationToken cancellationToken = default)
        => GaussDBSchema.GetSchema(async: true, this, collectionName, restrictions, cancellationToken);
    ...
    object ICloneable.Clone()
    {
        ...
        // `new GaussDBConnection(_connectionString)`, since that would bring in the default data source builder, and with it various
...
#pragma warning restore CS0618
    ...
    /// database or pooling)
    /// </summary>
    public GaussDBConnection CloneWith(string connectionString)
    {
        CheckDisposed();
        ...
        return new GaussDBConnection(csb.ToString())
        {
...
#pragma warning restore CS0618
    ...
    /// database or pooling)
    /// </summary>
    public async ValueTask<GaussDBConnection> CloneWithAsync(string connectionString, CancellationToken cancellationToken = default)
    {
        CheckDisposed();
        var csb = new GaussDBConnectionStringBuilder(connectionString);
...
#pragma warning restore CS0618
    ...
    /// DB provider factory.
    /// </summary>
    protected override DbProviderFactory DbProviderFactory => GaussDBFactory.Instance;
    ...
    /// </summary>
    public static void ClearPool(GaussDBConnection connection) => PoolManager.Clear(connection._connectionString);
    ...
    /// <summary>
    /// This event is unsupported by GaussDB. Use <see cref="DbConnection.StateChange"/> instead.
    /// </summary>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public new event EventHandler? Disposed
    {
        add => throw new NotSupportedException("The Disposed event isn't supported by GaussDB. Use DbConnection.StateChange instead.");
        remove => throw new NotSupportedException("The Disposed event isn't supported by GaussDB. Use DbConnection.StateChange instead.");
    }
...
readonly struct EndScopeDisposable(GaussDBConnection connection) : IDisposable
{
...
/// <summary>
/// Represents a method that handles the <see cref="GaussDBConnection.Notice"/> event.
/// </summary>
/// <param name="sender">The source of the event.</param>
/// <param name="e">A <see cref="GaussDBNoticeEventArgs"/> that contains the notice information (e.g. message, severity...).</param>
public delegate void NoticeEventHandler(object sender, GaussDBNoticeEventArgs e);

/// <summary>
/// Represents a method that handles the <see cref="GaussDBConnection.Notification"/> event.
/// </summary>
/// <param name="sender">The source of the event.</param>
/// <param name="e">A <see cref="GaussDBNotificationEventArgs"/> that contains the notification payload.</param>
public delegate void NotificationEventHandler(object sender, GaussDBNotificationEventArgs e);
