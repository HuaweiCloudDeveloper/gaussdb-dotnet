...
///</summary>
public class GaussDBParameter : DbParameter, IDbDataParameter, ICloneable
{
    ...
    internal GaussDBDbType? _gaussdbDbType;
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="GaussDBParameter"/> class.
    /// </summary>
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="GaussDBParameter"/> class with the parameter name and a value.
    /// </summary>
    /// <param name="parameterName">The name of the parameter to map.</param>
    /// <param name="value">The value of the <see cref="GaussDBParameter"/>.</param>
    /// <remarks>
    /// <p>
    ...
    /// </p>
    /// </remarks>
    public GaussDBParameter(string? parameterName, object? value)
        : this()
    {
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="GaussDBParameter"/> class with the parameter name and the data type.
    /// </summary>
    /// <param name="parameterName">The name of the parameter to map.</param>
    /// <param name="parameterType">One of the <see cref="GaussDBTypes.GaussDBDbType"/> values.</param>
    public GaussDBParameter(string? parameterName, GaussDBDbType parameterType)
        : this(parameterName, parameterType, 0, string.Empty)
    {
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="GaussDBParameter"/>.
    /// </summary>
    /// <param name="parameterName">The name of the parameter to map.</param>
    /// <param name="parameterType">One of the <see cref="System.Data.DbType"/> values.</param>
    public GaussDBParameter(string? parameterName, DbType parameterType)
        : this(parameterName, parameterType, 0, string.Empty)
    {
    }
    ...
    public GaussDBParameter(string? parameterName, GaussDBDbType parameterType, int size)
    ...
    /// <param name="parameterType">One of the <see cref="System.Data.DbType"/> values.</param>
    /// <param name="size">The length of the parameter.</param>
    public GaussDBParameter(string? parameterName, DbType parameterType, int size)
        : this(parameterName, parameterType, size, string.Empty)
    {
    }
    ...
    public GaussDBParameter(string? parameterName, GaussDBDbType parameterType, int size, string? sourceColumn)
    {
        ParameterName = parameterName;
        GaussDBDbType = parameterType;
        _size = size;
    ...
    /// <param name="size">The length of the parameter.</param>
    /// <param name="sourceColumn">The name of the source column.</param>
    public GaussDBParameter(string? parameterName, DbType parameterType, int size, string? sourceColumn)
    {
        ParameterName = parameterName;
    ...
    /// <param name="value">An <see cref="object"/> that is the value of the <see cref="GaussDBParameter"/>.</param>
    public GaussDBParameter(string parameterName, GaussDBDbType parameterType, int size, string? sourceColumn,
    ...
    /// <summary>
    /// Gets or sets The name of the <see cref="GaussDBParameter"/>.
    /// </summary>
    /// <value>The name of the <see cref="GaussDBParameter"/>.
    ...
    public sealed override DbType DbType
    {
        get
        {
            if (_gaussdbDbType is { } gaussdbDbType)
                return gaussdbDbType.ToDbType();

            if (_dataTypeName is not null)
                return Internal.Postgres.DataTypeName.FromDisplayName(_dataTypeName).ToGaussDBDbType()?.ToDbType() ?? DbType.Object;
            ...
            if (Value is not null)
                ...
                return GlobalTypeMapper.Instance.FindDataTypeName(GetValueType(StaticValueType)!, Value)?.ToGaussDBDbType()?.ToDbType() ?? DbType.Object;
        ...
        {
            ResetTypeInfo();
            _gaussdbDbType = value == DbType.Object
                ? null
                : value.ToGaussDBDbType()
                  ?? throw new NotSupportedException($"The parameter type DbType.{value} isn't supported by PostgreSQL or GaussDB");
        }
    }

    /// <summary>
    /// Gets or sets the <see cref="GaussDBTypes.GaussDBDbType"/> of the parameter.
    /// </summary>
    /// <value>One of the <see cref="GaussDBTypes.GaussDBDbType"/> values. The default is <see cref="GaussDBTypes.GaussDBDbType"/>.</value>
    [DefaultValue(GaussDBDbType.Unknown)]
    [Category("Data"), RefreshProperties(RefreshProperties.All)]
    [DbProviderSpecificTypeProperty(true)]
    public GaussDBDbType GaussDBDbType
    {
        get
        {
            if (_gaussdbDbType.HasValue)
                return _gaussdbDbType.Value;

            if (_dataTypeName is not null)
                return Internal.Postgres.DataTypeName.FromDisplayName(_dataTypeName).ToGaussDBDbType() ?? GaussDBDbType.Unknown;

            // Infer from value but don't cache
            if (Value is not null)
                // We pass ValueType here for the generic derived type (GaussDBParameter<T>) where we should respect T and not the runtime type.
                return GlobalTypeMapper.Instance.FindDataTypeName(GetValueType(StaticValueType)!, Value)?.ToGaussDBDbType() ?? GaussDBDbType.Unknown;

            return GaussDBDbType.Unknown;
        }
        set
        {
            if (value == GaussDBDbType.Array)
                ThrowHelper.ThrowArgumentOutOfRangeException(nameof(value), "Cannot set GaussDBDbType to just Array, Binary-Or with the element type (e.g. Array of Box is GaussDBDbType.Array | GaussDBDbType.Box).");
            if (value == GaussDBDbType.Range)
                ThrowHelper.ThrowArgumentOutOfRangeException(nameof(value), "Cannot set GaussDBDbType to just Range, Binary-Or with the element type (e.g. Range of integer is GaussDBDbType.Range | GaussDBDbType.Integer)");

            ResetTypeInfo();
            _gaussdbDbType = value;
        }
    }
    ...
    public string? DataTypeName
    {
        get
        {
            ...
            if (_gaussdbDbType is { } gaussdbDbType)
            {
                var unqualifiedName = gaussdbDbType.ToUnqualifiedDataTypeName();
    ...
    /// </summary>
    public GaussDBParameterCollection? Collection { get; set; }
    ...
    /// <see cref="GaussDBCommandBuilder.DeriveParameters"/> and can be used to
    ...
    internal void ResolveTypeInfo(PgSerializerOptions options)
    {
        ...
        {
            var dataTypeName =
                _gaussdbDbType is { } gaussdbDbType
                    ? gaussdbDbType.ToDataTypeName() ?? gaussdbDbType.ToUnqualifiedDataTypeNameOrThrow()
            ...
            if (!unspecifiedDBNull)
                typeInfo = AdoSerializerHelpers.GetTypeInfoForWriting(valueType, pgTypeId, options, _gaussdbDbType);
        ...
        void ThrowNoTypeInfo()
            => ThrowHelper.ThrowInvalidOperationException(
                $"Parameter '{(!string.IsNullOrEmpty(ParameterName) ? ParameterName : $"${Collection?.IndexOf(this) + 1}")}' must have either its GaussDBDbType or its DataTypeName or its Value set.");

        void ThrowNotSupported(string dataTypeName)
        {
            ThrowHelper.ThrowNotSupportedException(_gaussdbDbType is not null
                ? $"The GaussDBDbType '{_gaussdbDbType}' isn't present in your database. You may need to install an extension or upgrade to a newer version."
    ...
    /// <summary>
    /// Creates a new <see cref="GaussDBParameter"/> that is a copy of the current instance.
    /// </summary>
    /// <returns>A new <see cref="GaussDBParameter"/> that is a copy of this instance.</returns>
    public GaussDBParameter Clone() => CloneCore();

    private protected virtual GaussDBParameter CloneCore() =>
