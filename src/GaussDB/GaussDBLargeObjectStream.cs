...
/// </summary>
[Obsolete("GaussDBLargeObjectStream allows manipulating PostgreSQL large objects via publicly available PostgreSQL functions (lo_read, lo_write); call these yourself directly.")]
public sealed class GaussDBLargeObjectStream : Stream
{
    readonly GaussDBLargeObjectManager _manager;
    readonly int _fd;
    long _pos;
    readonly bool _writeable;
    bool _disposed;

    internal GaussDBLargeObjectStream(GaussDBLargeObjectManager manager, int fd, bool writeable)
    {
        _manager = manager;
        _fd = fd;
        _pos = 0;
    ...
    async Task Write(bool async, byte[] buffer, int offset, int count, CancellationToken cancellationToken = default)
    {
        ...
        while (totalWritten < count)
        {
            ...
            if (bytesWritten != chunkSize)
                throw new InvalidOperationException($"Internal GaussDB bug, please report");
