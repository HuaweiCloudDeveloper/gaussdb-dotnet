...
/// Can be retrieved using <see cref="GaussDBDataReader.GetData(int)"/> or
/// <see cref="GaussDBNestedDataReader.GetData(int)"/>.
/// </summary>
public sealed class GaussDBNestedDataReader : DbDataReader
{
    readonly GaussDBDataReader _outermostReader;
    readonly GaussDBNestedDataReader? _outerNestedReader;
    GaussDBNestedDataReader? _cachedFreeNestedDataReader;
    ...
    internal GaussDBNestedDataReader(GaussDBDataReader outermostReader, GaussDBNestedDataReader? outerNestedReader,
    ...
    public new GaussDBNestedDataReader GetData(int ordinal)
    {
        ...
        {
            reader = new GaussDBNestedDataReader(_outermostReader, this, _depth + 1, compositeType);
        }
        if (isArray)
            reader.InitArray();
        else
    ...
    /// <inheritdoc />
    [UnconditionalSuppressMessage("ILLink", "IL2093", Justification = "No members are dynamically accessed by GaussDB via GaussDBNestedDataReader.GetFieldType.")]
