...
sealed class GaussDBBufferWriter(GaussDBWriteBuffer buffer) : IStreamingWriter<byte>
{
    int? _lastBufferSize;
...
public sealed class PgWriter
{
    ...
    GaussDBDatabaseInfo? _typeCatalog;

    internal PgWriter(IBufferWriter<byte> writer) => _writer = writer;

    internal PgWriter Init(GaussDBDatabaseInfo typeCatalog, FlushMode flushMode = FlushMode.None)
    {
        if (_pos != _offset)
