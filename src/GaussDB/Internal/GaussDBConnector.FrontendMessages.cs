...
partial class GaussDBConnector
{
    ...
    {
        GaussDBWriteBuffer.AssertASCIIOnly(asciiName);
        ...
        static void Write(GaussDBWriteBuffer writeBuffer, int len, StatementOrPortal statementOrPortal, byte[] name)
        {
    ...
    internal Task WriteSync(bool async, CancellationToken cancellationToken = default)
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void Write(GaussDBWriteBuffer writeBuffer)
        {
    ...
    internal Task WriteExecute(int maxRows, bool async, CancellationToken cancellationToken = default)
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void Write(GaussDBWriteBuffer writeBuffer, int maxRows)
        {
    ...
    internal async Task WriteParse(string sql, byte[] asciiName, List<GaussDBParameter> inputParameters, bool async, CancellationToken cancellationToken = default)
    {
        GaussDBWriteBuffer.AssertASCIIOnly(asciiName);
    ...
    internal async Task WriteBind(
        List<GaussDBParameter> parameters,
        string portal,
        byte[] asciiName,
    ...
    {
        GaussDBWriteBuffer.AssertASCIIOnly(asciiName);
        GaussDBWriteBuffer.AssertASCIIOnly(portal);
    ...
    internal Task WriteClose(StatementOrPortal type, byte[] asciiName, bool async, CancellationToken cancellationToken = default)
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void Write(GaussDBWriteBuffer writeBuffer, int len, StatementOrPortal type, byte[] name)
        {
    ...
    internal void WriteStartup(Dictionary<string, string> parameters)
    {
        ...
        foreach (var kvp in parameters)
            len += GaussDBWriteBuffer.UTF8Encoding.GetByteCount(kvp.Key) + 1 +
                   GaussDBWriteBuffer.UTF8Encoding.GetByteCount(kvp.Value) + 1;
