...
/// <summary>
/// A buffer used by GaussDB to write data to the socket efficiently.
/// Provides methods which encode different values types and tracks the current position.
/// </summary>
sealed class GaussDBWriteBuffer : IDisposable
{
    ...
    internal PgWriter GetWriter(GaussDBDatabaseInfo typeCatalog, FlushMode flushMode = FlushMode.None)
        ...
        Encoding textEncoding)
    {
        ...
        _pgWriter = new PgWriter(new GaussDBBufferWriter(this));
    }
    ...
    public async Task Flush(bool async, CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception ex)
        {
            ...
            case IOException { InnerException: SocketException { SocketErrorCode: SocketError.TimedOut } }:
                ...
                throw Connector.Break(new GaussDBException("Exception while writing to stream", new TimeoutException("Timeout during writing attempt")));
            }

            throw Connector.Break(new GaussDBException("Exception while writing to stream", ex));
        }
        GaussDBEventSource.Log.BytesWritten(WritePosition);
    ...
    internal void DirectWrite(ReadOnlySpan<byte> buffer)
    {
        ...
        {
            throw Connector.Break(new GaussDBException("Exception while writing to stream", e));
        }
    }
    ...
    public Task WriteString(string s, int charLen, int byteLen, bool async, CancellationToken cancellationToken = default)
    {
        ...
        static async Task WriteStringLong(GaussDBWriteBuffer buffer, bool async, string s, int charLen, int byteLen, CancellationToken cancellationToken)
        {
    ...
    public Task WriteBytesRaw(ReadOnlyMemory<byte> bytes, bool async, CancellationToken cancellationToken = default)
    {
        ...
        static async Task WriteBytesLong(GaussDBWriteBuffer buffer, bool async, ReadOnlyMemory<byte> bytes, CancellationToken cancellationToken)
        {
