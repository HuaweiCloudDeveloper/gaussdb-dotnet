...
public class PgReader
{
    ...
    readonly GaussDBReadBuffer _buffer;

    bool _resumable;

    byte[]? _pooledArray;
    GaussDBReadBuffer.ColumnStream? _userActiveStream;
    ...
    internal PgReader(GaussDBReadBuffer buffer)
    {
        _buffer = buffer;
    ...
    // TODO refactor out
    internal long GetFieldStartPos(GaussDBNestedDataReader nestedDataReader) => _fieldStartPos;
    // TODO refactor out
    internal int GetFieldOffset(GaussDBNestedDataReader nestedDataReader) => FieldOffset;
    ...
    {
        if (GaussDBReadBuffer.BufferBoundsChecks)
            Core(count);
    ...
    GaussDBReadBuffer.ColumnStream GetColumnStream(bool canSeek = false, int? length = null)
    {
