...
partial class GaussDBConnector
{
    async Task Authenticate(string username, GaussDBTimeout timeout, bool async, CancellationToken cancellationToken)
    {
        ...
        if (requiredAuthModes == default)
            requiredAuthModes = GaussDBConnectionStringBuilder.ParseAuthMode(PostgresEnvironment.RequireAuth);
        ...
        static void ThrowIfNotAllowed(RequireAuthMode requiredAuthModes, RequireAuthMode requestedAuthMode)
        {
            if (!requiredAuthModes.HasFlag(requestedAuthMode))
                throw new GaussDBException($"\"{requestedAuthMode}\" authentication method is not allowed. Allowed methods: {requiredAuthModes}");
        }
    }
    ...
    async Task AuthenticateCleartext(string username, bool async, CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrEmpty(passwd))
            throw new GaussDBException("No password has been provided but the backend requires one (in cleartext)");
    ...
    async Task AuthenticateSHA256(string username, AuthenticationSHA256PasswordMessage message, bool async, CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrEmpty(password))
            throw new GaussDBException("No password has been provided but the backend requires one (in SHA256)");

        var normalizedPassword = password.Normalize(NormalizationForm.FormKC);
        var passwordBytes = GaussDBWriteBuffer.UTF8Encoding.GetBytes(normalizedPassword);
    ...
    async Task AuthenticateMD5SHA256(string username, AuthenticationMD5SHA256PasswordMessage message, bool async, CancellationToken cancellationToken = default)
    {
        ...
        var passwordBytes = GaussDBWriteBuffer.UTF8Encoding.GetBytes(password);
        ...
        using (var md5 = MD5.Create())
        {
            ...
            var stringBytes = GaussDBWriteBuffer.UTF8Encoding.GetBytes(encryptedString);
