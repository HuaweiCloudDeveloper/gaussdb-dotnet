...
sealed partial class GaussDBReadBuffer : IDisposable
{
    ...
    public GaussDBConnection Connection => Connector.Connection!;
    internal readonly GaussDBConnector Connector;
    ...
    int ReadWithTimeout(Span<byte> buffer)
    {
        while (true)
        {
            try
            {
                ...
                GaussDBEventSource.Log.BytesRead(read);
                return read;
            }
            catch (Exception ex)
            {
                ...
                throw connector.Break(new GaussDBException("Exception while reading from stream", ex));
            }
        }
    }
    ...
    static Exception CreateCancelException(GaussDBConnector connector)
        => !connector.UserCancellationRequested
            ? GaussDBTimeoutException()
    ...
    static Exception GaussDBTimeoutException() => new GaussDBException("Exception while reading from stream", TimeoutException());
            ...
            while (count > 0)
            {
                ...
                catch (Exception e)
                {
                                            ...
                                            (Type.GetType("Mono.Runtime") == null ? SocketError.TimedOut : SocketError.WouldBlock):
                    {
                        ...
                        static Exception CreateException(GaussDBConnector connector)
                            => !connector.UserCancellationRequested
                                ? GaussDBTimeoutException()
                                : connector.PostgresCancellationPerformed
                                    ? new OperationCanceledException("Query was cancelled", TimeoutException(), connector.UserCancellationToken)
                    ...
                    default:
                        throw connector.Break(new GaussDBException("Exception while reading from stream", e));
                    }
                }
            }

            buffer.Cts.Stop();
            GaussDBEventSource.Log.BytesRead(totalRead);
    ...
    internal GaussDBReadBuffer AllocateOversize(int count)
    {
        Debug.Assert(count > Size);
        var tempBuf = new GaussDBReadBuffer(Connector, Underlying, _underlyingSocket, count, TextEncoding, RelaxedTextEncoding, usePool: true);
    ...
    public ValueTask<int> ReadAsync(bool commandScoped, Memory<byte> output, CancellationToken cancellationToken = default)
    {
        ...
        static async ValueTask<int> ReadAsyncLong(GaussDBReadBuffer buffer, bool commandScoped, Memory<byte> output, CancellationToken cancellationToken)
        {
            // Only reset if we'll be able to read data, this is to support zero-byte reads.
            if (output.Length > 0)
            {
            ...
            {
                throw buffer.Connector.Break(new GaussDBException("Exception while reading from stream", e));
            }
        }
    }
    ...
    internal void CopyTo(GaussDBReadBuffer other)
    {
