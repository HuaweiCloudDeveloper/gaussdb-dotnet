...
/// exposed to users, connectors are internal to GaussDB and are recycled by the connection pool.
/// </summary>
[Experimental(GaussDBDiagnostics.ConvertersExperimental)]
public sealed partial class GaussDBConnector
{
    ...
    /// </summary>
    public GaussDBConnectionStringBuilder Settings { get; }
...
#pragma warning restore CS0618
    ...
    /// </summary>
    internal GaussDBReadBuffer ReadBuffer { get; private set; } = default!;
    ...
    /// </summary>
    GaussDBReadBuffer? _origReadBuffer;

    /// <summary>
    /// Buffer used for writing data.
    /// </summary>
    internal GaussDBWriteBuffer WriteBuffer { get; private set; } = default!;
    ...
    /// </summary>
    public GaussDBDatabaseInfo DatabaseInfo { get; internal set; } = default!;
    ...
    /// </summary>
    internal GaussDBTransaction? Transaction { get; set; }

    internal GaussDBTransaction? UnboundTransaction { get; set; }

    /// <summary>
    /// The GaussDBConnection that (currently) owns this connector. Null if the connector isn't
    /// owned (i.e. idle in the pool)
    /// </summary>
    internal GaussDBConnection? Connection { get; set; }
    ...
    internal GaussDBDataReader? CurrentReader;
    ...
    /// </summary>
    GaussDBCommand? _currentCommand;
    ...
    /// <see cref="GaussDBMultiHostDataSource"/>.
    /// </summary>
    internal GaussDBDataSource DataSource { get; }
    ...
    /// <see cref="GaussDBConnectionStringBuilder.ConnectionLifetime"/>.
    /// </summary>
...
#pragma warning restore CA1859
    ...
    internal GaussDBLoggingConfiguration LoggingConfiguration { get; }
    ...
    internal GaussDBDataReader DataReader { get; set; }

    internal GaussDBDataReader? UnboundDataReader { get; set; }

    #region Constructors

    internal GaussDBConnector(GaussDBDataSource dataSource, GaussDBConnection conn)
        : this(dataSource)
    {
...
#pragma warning restore CS0618
    ...
    GaussDBConnector(GaussDBConnector connector)
    ...
    GaussDBConnector(GaussDBDataSource dataSource)
    {
        ...
        DataReader = new GaussDBDataReader(this);
        ...
        if (Settings.Multiplexing)
        {
            ...
            var commandsInFlightChannel = Channel.CreateUnbounded<GaussDBCommand>(
    ...
    internal async Task Open(GaussDBTimeout timeout, bool async, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            activity = GaussDBActivitySource.ConnectionOpen(this);
        ...
        {
            if (activity is not null)
                GaussDBActivitySource.SetException(activity, e);
            Break(e);
            throw;
        }

        static async Task OpenCore(
            GaussDBConnector conn,
            string username,
            SslMode sslMode,
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (msg.Code != BackendMessageCode.ReadyForQuery)
                throw new GaussDBException($"Received backend message {msg.Code} while expecting ReadyForQuery. Please file a bug.");
        ...
        GaussDBTimeout timeout, bool async, CancellationToken cancellationToken = default)
    {
        ...
        batch.BatchCommands.Add(new GaussDBBatchCommand("select pg_is_in_recovery()"));
        batch.BatchCommands.Add(new GaussDBBatchCommand("SHOW default_transaction_read_only"));
    ...
    ValueTask<string> GetUsernameAsync(bool async, CancellationToken cancellationToken)
    {
        ...
        async ValueTask<string> GetUsernameAsyncInternal()
        {
            ...
            throw new GaussDBException("No username could be found, please specify one explicitly");
        }
    }

    async Task RawOpen(SslMode sslMode, GaussDBTimeout timeout, bool async, CancellationToken cancellationToken)
    {
        try
        {
            if (async)
            ...
            {
                TextEncoding = GaussDBWriteBuffer.UTF8Encoding;
                RelaxedTextEncoding = GaussDBWriteBuffer.RelaxedUTF8Encoding;
            }
            else
            {
            ...
            ReadBuffer = new GaussDBReadBuffer(this, _stream, _socket, Settings.ReadBufferSize, TextEncoding, RelaxedTextEncoding);
            WriteBuffer = new GaussDBWriteBuffer(this, _stream, _socket, Settings.WriteBufferSize, TextEncoding);
            ...
            {
                // We already check that in GaussDBConnectionStringBuilder.PostProcessAndValidate, but since we also allow environment variables...
                ...
                if (ReadBuffer.ReadBytesLeft > 0)
                    throw new GaussDBException("Additional unencrypted data received after SSL negotiation - this should never happen, and may be an indication of a man-in-the-middle attack.");
            }
                ...
                sslMode is SslMode.Require or SslMode.VerifyCA or SslMode.VerifyFull)
            {
                ...
                {
                default:
                    throw new GaussDBException($"Received unknown response {response} for SSLRequest (expecting S or N)");
                case 'N':
                    if (sslMode != SslMode.Prefer)
                        throw new GaussDBException("SSL connection requested. No SSL enabled connection from this host is configured.");
                    break;
                case 'S':
                    await DataSource.TransportSecurityHandler.NegotiateEncryption(async, this, sslMode, timeout, cancellationToken).ConfigureAwait(false);
                    break;
    ...
    static SslNegotiation GetSslNegotiation(GaussDBConnectionStringBuilder settings)
    {
    ...
    internal async Task NegotiateEncryption(SslMode sslMode, GaussDBTimeout timeout, bool async, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (SslClientAuthenticationOptionsCallback is not null)
            {
                ...
                if (sslStreamOptions.RemoteCertificateValidationCallback != certificateValidationCallback)
                {
                    if (sslMode is SslMode.VerifyCA or SslMode.VerifyFull)
                        throw new ArgumentException(string.Format(GaussDBStrings.CannotUseSslVerifyWithCustomValidationCallback, sslMode));

                    if (Settings.RootCertificate is not null)
                        throw new ArgumentException(GaussDBStrings.CannotUseSslRootCertificateWithCustomValidationCallback);

                    if (DataSource.TransportSecurityHandler.RootCertificateCallback is not null)
                        throw new ArgumentException(GaussDBStrings.CannotUseValidationRootCertificateCallbackWithCustomValidationCallback);
                }
            }
            ...
            {
                sslStream.Dispose();
                throw new GaussDBException("Exception while performing SSL handshake", e);
            }
    ...
    void Connect(GaussDBTimeout timeout)
    {
        // Note that there aren't any timeout-able or cancellable DNS methods
        var endpoints = GaussDBConnectionStringBuilder.IsUnixSocket(Host, Port, out var socketPath)
        ...
        for (var i = 0; i < endpoints.Length; i++)
        {
            ...
            catch (Exception e)
            {
                ...
                if (i == endpoints.Length - 1)
                    throw new GaussDBException($"Failed to connect to {endpoint}", e);
            }
        }
    }

    async Task ConnectAsync(GaussDBTimeout timeout, CancellationToken cancellationToken)
    {
        ...
        for (var i = 0; i < endpoints.Length; i++)
        {
            var endpointTimeout = timeout.IsSet ? new GaussDBTimeout(perEndpointTimeout) : timeout;
        ...
        static Task OpenSocketConnectionAsync(Socket socket, EndPoint endpoint, GaussDBTimeout perIpTimeout, CancellationToken cancellationToken)
        {
    ...
    readonly ChannelReader<GaussDBCommand>? CommandsInFlightReader;
    internal readonly ChannelWriter<GaussDBCommand>? CommandsInFlightWriter;
        ...
        try
        {
            while (await CommandsInFlightReader.WaitToReadAsync().ConfigureAwait(false))
            {
                ...
                while (CommandsInFlightReader.TryRead(out command))
                {
                    ...
                    // true, so that the user code calling GaussDBDataReader.Dispose will not continue executing
        ...
        catch (Exception e)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    pendingCommand.ExecutionCompletion.SetException(new GaussDBException("A previous command on this connection caused an error requiring all pending commands on this connection to be aborted", _breakReason!));
                }
            }
        ...
        try
        {
            while (true)
            {
                ...
                case BackendMessageCode.ReadyForQuery:
                    if (error != null)
                    {
                        GaussDBEventSource.Log.CommandFailed();
        ...
        catch (PostgresException e)
        {
            if (e.SqlState == PostgresErrorCodes.QueryCanceled && PostgresCancellationPerformed)
            {
                ...
                throw UserCancellationRequested
                    ...
                    : new GaussDBException("Exception while reading from stream",
                        new TimeoutException("Timeout during reading attempt"));
            }

            throw;
        }
        catch (GaussDBException)
        {
    ...
    internal IBackendMessage? ParseResultSetMessage(GaussDBReadBuffer buf, BackendMessageCode code, int len, bool handleCallbacks = false)
        => code switch
        {
    ...
    internal IBackendMessage? ParseServerMessage(GaussDBReadBuffer buf, BackendMessageCode code, int len, bool isPrependedMessage, bool handleCallbacks = true)
    {
        switch (code)
        {
        ...
        case BackendMessageCode.NotificationResponse:
            if (handleCallbacks)
            {
                Connection?.OnNotification(new GaussDBNotificationEventArgs(buf));
            }
            return null;
    ...
    internal bool InTransaction
    {
        get
        {
            ...
            case TransactionStatus.InFailedTransactionBlock:
                return true;
            default:
                ThrowHelper.ThrowInvalidOperationException($"Internal GaussDB bug: unexpected value {{0}} of enum {nameof(TransactionStatus)}. Please file a bug.", TransactionStatus);
                return false;
            }
        }
    }
    ...
    void ProcessNewTransactionStatus(TransactionStatus newStatus)
    {
        ...
        case TransactionStatus.InFailedTransactionBlock:
            ...
            return;
        case TransactionStatus.Pending:
            ThrowHelper.ThrowInvalidOperationException($"Internal GaussDB bug: invalid TransactionStatus {nameof(TransactionStatus.Pending)} received, should be frontend-only");
            return;
        default:
            ThrowHelper.ThrowInvalidOperationException($"Internal GaussDB bug: unexpected value {{0}} of enum {nameof(TransactionStatus)}. Please file a bug.", newStatus);
            return;
        }
    }
    ...
    internal bool PerformPostgresCancellation()
    {
        ...
        lock (CancelLock)
        {
            ...
            try
            {
                var cancelConnector = new GaussDBConnector(this);
    ...
    void DoCancelRequest(int backendProcessId, int backendSecretKey)
    {
        ...
        try
        {
            RawOpen(Settings.SslMode, new GaussDBTimeout(TimeSpan.FromSeconds(ConnectionTimeout)), false, CancellationToken.None)
        ...
        bool attemptPgCancellation = true)
    {
        ...
        return _cancellationTokenRegistration =
            cancellationToken.Register(static c => ((GaussDBConnector)c!).PerformImmediateUserCancellation(), this);
    }
    ...
    /// <see cref="GaussDBConnectionStringBuilder.CancellationTimeout"/> has elapsed. If <see langword="false" />,
        ...
        bool attemptPgCancellation = true)
    {
        ...
        var registration = cancellationToken.Register(static c => ((GaussDBConnector)c!).PerformImmediateUserCancellation(), this);
    ...
    internal async Task CloseOngoingOperations(bool async)
    {
        ...
        else if (copyOperation != null)
        {
            ...
            if (copyOperation is GaussDBBinaryImporter ||
                copyOperation is GaussDBCopyTextWriter ||
                copyOperation is GaussDBRawCopyStream { CanWrite: true })
            {
                try
                {
                    if (async)
    ...
    internal Exception Break(Exception reason)
    {
        ...
        try
        {
            ...
            lock (CancelLock)
            {
                ...
                if (reason is GaussDBException { IsTransient: true } ne &&
    ...
    internal async Task Reset(bool async)
    {
        ...
        using (StartUserAction(attemptPgCancellation: false))
        {
            ...
            case TransactionStatus.InFailedTransactionBlock:
                ...
                break;
            default:
                ThrowHelper.ThrowInvalidOperationException($"Internal GaussDB bug: unexpected value {TransactionStatus} of enum {nameof(TransactionStatus)}. Please file a bug.");
                return;
            }
    ...
    /// <param name="command">
    /// The <see cref="GaussDBCommand" /> that is starting execution - if an <see cref="GaussDBOperationInProgressException" /> is
    ...
    /// occurs, followed by a client-side socket cancellation once <see cref="GaussDBConnectionStringBuilder.CancellationTimeout"/> has
        ...
        bool attemptPgCancellation = true)
    {
        ...
        UserAction DoStartUserAction(ConnectorState newState, GaussDBCommand? command,
            CancellationToken cancellationToken, bool attemptPgCancellation)
        {
            ...
            case ConnectorState.Copy:
                ...
                if (currentCommand is null)
                    ThrowHelper.ThrowGaussDBOperationInProgressException(State);
                else
                    ThrowHelper.ThrowGaussDBOperationInProgressException(currentCommand);
                break;
            default:
        ...
        UserAction DoStartUserActionWithKeepAlive(ConnectorState newState, GaussDBCommand? command,
            CancellationToken cancellationToken, bool attemptPgCancellation)
        {
            lock (SyncObj)
            {
                ...
                {
                    if (IsBroken)
                        ThrowHelper.ThrowGaussDBException("The connection was previously broken because of the following exception", _breakReason);
                    else
                        ThrowHelper.ThrowGaussDBException("The connection is closed");
                }
                ...
                catch (Exception ex) when (ex is not GaussDBOperationInProgressException)
                {
    ...
    internal readonly struct UserAction : IDisposable
    {
        ...
        internal UserAction(GaussDBConnector connector) => _connector = connector;
    ...
    void PerformKeepAlive(object? state)
    {
        ...
        catch (Exception e)
        {
            ...
            try
            {
                Break(new GaussDBException("Exception while sending a keepalive", e));
            }
            catch (Exception e2)
            {
    ...
    internal async Task<bool> Wait(bool async, int timeout, CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            try
            {
                ...
                {
                    throw Break(
                        new GaussDBException($"Received unexpected message of type {msg.Code} while waiting"));
                }
                return true;
            }
            catch (GaussDBException e) when (e.InnerException is TimeoutException)
            {
            ...
            while (true)
            {
                ...
                catch (Exception e) when (e is OperationCanceledException || e is GaussDBException { InnerException: TimeoutException })
                {
                ...
                if (msg.Code != BackendMessageCode.ReadyForQuery)
                    throw new GaussDBException($"Received unexpected message of type {msg.Code} while expecting {expectedMessageCode} as part of keepalive");
    ...
    /// <summary>
    /// Creates and returns a <see cref="GaussDBCommand"/> object associated with the <see cref="GaussDBConnector"/>.
    /// </summary>
    /// <param name="cmdText">The text of the query.</param>
    /// <returns>A <see cref="GaussDBCommand"/> object.</returns>
    public GaussDBCommand CreateCommand(string? cmdText = null) => new(cmdText, this);

    /// <summary>
    /// Creates and returns a <see cref="GaussDBBatch"/> object associated with the <see cref="GaussDBConnector"/>.
    /// </summary>
    /// <returns>A <see cref="GaussDBBatch"/> object.</returns>
    public GaussDBBatch CreateBatch() => new GaussDBBatch(this);
