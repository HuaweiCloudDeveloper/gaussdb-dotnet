...
/// </summary>
[Experimental(GaussDBDiagnostics.DatabaseInfoExperimental)]
public abstract class GaussDBDatabaseInfo
{
    #region Fields

    static volatile IGaussDBDatabaseInfoFactory[] Factories =
    [
    ...
    /// Exposed via <see cref="GaussDBConnection.PostgreSqlVersion"/>.
    /// </summary>
    ...
    /// <summary>
    /// Initializes the instance of <see cref="GaussDBDatabaseInfo"/>.
    /// </summary>
    protected GaussDBDatabaseInfo(string host, int port, string databaseName, Version version)
    ...
    /// Initializes the instance of <see cref="GaussDBDatabaseInfo"/>.
    /// </summary>
    protected GaussDBDatabaseInfo(string host, int port, string databaseName, Version version, string serverVersion)
    {
        Host = host;
        Port = port;
        Name = databaseName;
    ...
    private protected GaussDBDatabaseInfo(string host, int port, string databaseName, string serverVersion)
    {
        Host = host;
        Port = port;
        Name = databaseName;
    ...
    /// </summary>
    public static void RegisterFactory(IGaussDBDatabaseInfoFactory factory)
    {
        ArgumentNullException.ThrowIfNull(factory);

        var factories = new IGaussDBDatabaseInfoFactory[Factories.Length + 1];
        factories[0] = factory;
        Array.Copy(Factories, 0, factories, 1, Factories.Length);
        Factories = factories;
    }

    internal static async Task<GaussDBDatabaseInfo> Load(GaussDBConnector conn, GaussDBTimeout timeout, bool async)
    {
        ...
        throw new GaussDBException("No DatabaseInfoFactory could be found for this connection");
    }
