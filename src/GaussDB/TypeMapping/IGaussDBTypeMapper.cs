...
/// </summary>
/// <remarks>
/// The preferred way to manage type mappings is on <see cref="GaussDBDataSourceBuilder" />. An alternative, but discouraged, method, is to
/// manage them globally via <see cref="GaussDBConnection.GlobalTypeMapper"/>).
/// </remarks>
public interface IGaussDBTypeMapper
{
    /// <summary>
    /// The default name translator to convert CLR type names and member names. Defaults to <see cref="GaussDBSnakeCaseNameTranslator"/>.
    /// </summary>
    IGaussDBNameTranslator DefaultNameTranslator { get; set; }
    ...
    IGaussDBTypeMapper MapEnum<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicFields)] TEnum>(
        string? pgName = null,
    ...
    IGaussDBTypeMapper MapComposite<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.PublicProperties | DynamicallyAccessedMemberTypes.PublicFields)] T>(
        string? pgName = null,
        IGaussDBNameTranslator? nameTranslator = null);
    ...
    /// <param name="nameTranslator">
    /// A component which will be used to translate CLR names (e.g. SomeClass) into database names (e.g. some_class).
    /// Defaults to <see cref="GaussDBSnakeCaseNameTranslator"/>
    /// </param>
    [RequiresDynamicCode("Mapping composite types involves serializing arbitrary types which can require creating new generic types or methods. This is currently unsupported with NativeAOT, vote on issue #5303 if this is important to you.")]
    ...
    /// </param>
    [RequiresDynamicCode("Mapping composite types involves serializing arbitrary types which can require creating new generic types or methods. This is currently unsupported with NativeAOT, vote on issue #5303 if this is important to you.")]
    IGaussDBTypeMapper MapComposite(
    ...
    /// <returns></returns>
    IGaussDBTypeMapper ConfigureJsonOptions(JsonSerializerOptions serializerOptions);
    ...
    IGaussDBTypeMapper EnableDynamicJson(Type[]? jsonbClrTypes = null, Type[]? jsonClrTypes = null);
    ...
    IGaussDBTypeMapper EnableRecordsAsTuples();
    ...
    IGaussDBTypeMapper EnableUnmappedTypes();
