...
sealed class PostgresMinimalDatabaseInfoFactory : IGaussDBDatabaseInfoFactory
{
    public Task<GaussDBDatabaseInfo?> Load(GaussDBConnector conn, GaussDBTimeout timeout, bool async)
        => Task.FromResult(
            !conn.DataSource.Configuration.TypeLoading.LoadTypes
                ? (GaussDBDatabaseInfo)new PostgresMinimalDatabaseInfo(conn)
                : null);
}
...
sealed class PostgresMinimalDatabaseInfo : PostgresDatabaseInfo
{
    ...
    internal PostgresMinimalDatabaseInfo(GaussDBConnector conn)
        : base(conn)
