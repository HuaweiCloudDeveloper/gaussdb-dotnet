...
/// </summary>
static class GaussDBSchema
{
    public static Task<DataTable> GetSchema(bool async, GaussDBConnection conn, string? collectionName, string?[]? restrictions, CancellationToken cancellationToken = default)
    {
        ...
        return collectionName.ToUpperInvariant() switch
        {
            ...
            // custom collections for gaussdb
    ...
    static GaussDBCommand BuildCommand(GaussDBConnection conn, StringBuilder query, string?[]? restrictions, params string[]? names)
        => BuildCommand(conn, query, restrictions, true, names);

    static GaussDBCommand BuildCommand(GaussDBConnection conn, StringBuilder query, string?[]? restrictions, bool addWhere, params string[]? names)
    {
        ...
        if (restrictions != null && names != null)
        {
            for (var i = 0; i < restrictions.Length && i < names.Length; ++i)
            {
                if (restrictions[i] is { Length: > 0 } restriction)
                {
                    ...
                    command.Parameters.Add(new GaussDBParameter(paramName, restriction));
                }
            }
        }
    ...
    static Task<DataTable> GetDatabases(GaussDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
""");
    ...
    static Task<DataTable> GetSchemata(GaussDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
""");
    ...
    static Task<DataTable> GetTables(GaussDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
""");
    ...
    static Task<DataTable> GetColumns(GaussDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
""");
    ...
    static Task<DataTable> GetViews(GaussDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
""");
    ...
    /*static Task<DataTable> GetMaterializedViews(GaussDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
    ...
    */
    ...
    static Task<DataTable> GetUsers(GaussDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
    ...
    static Task<DataTable> GetIndexes(GaussDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
""");
    ...
    static Task<DataTable> GetIndexColumns(GaussDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
""");
    ...
    static Task<DataTable> GetConstraints(GaussDBConnection conn, string?[]? restrictions, string? constraintType, bool async, CancellationToken cancellationToken = default)
    {
        var sql = new StringBuilder(
            """
SELECT
...
""");
    ...
    static Task<DataTable> GetConstraintColumns(GaussDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
        var sql = new StringBuilder(
            """
SELECT current_database(),
...
""");
    ...
    static DataTable GetDataSourceInformation(GaussDBConnection conn)
    {
        ...
        if (GaussDBCommand.EnableSqlRewriting)
        {
    ...
    static DataTable GetDataTypes(GaussDBConnection conn)
    {
        ...
        try
        {
            ...
            foreach (var baseType in types)
            {
                ...
                if (baseType.DataTypeName.ToGaussDBDbType() is { } gaussdbDbType)
                    row["ProviderDbType"] = (int)gaussdbDbType;
            }
            ...
            foreach (var arrayType in connector.DatabaseInfo.ArrayTypes)
            {
                ...
                row["CreateFormat"] += "[]";
                row["DataType"] = info.Type.FullName;
                if (arrayType.DataTypeName.ToGaussDBDbType() is { } gaussdbDbType)
                    row["ProviderDbType"] = (int)gaussdbDbType;
            }
            ...
            foreach (var rangeType in connector.DatabaseInfo.RangeTypes)
            {
                ...
                row["DataType"] = info.Type.FullName;
                if (rangeType.DataTypeName.ToGaussDBDbType() is { } gaussdbDbType)
                    row["ProviderDbType"] = (int)gaussdbDbType;
            }
            ...
            foreach (var multirangeType in connector.DatabaseInfo.MultirangeTypes)
            {
                ...
                row["DataType"] = info.Type.FullName;
                if (multirangeType.DataTypeName.ToGaussDBDbType() is { } gaussdbDbType)
                    row["ProviderDbType"] = (int)gaussdbDbType;
            }
            ...
            foreach (var domainType in connector.DatabaseInfo.DomainTypes)
            {
                ...
                row["DataType"] = info.Type.FullName;
                if (representationalType.DataTypeName.ToGaussDBDbType() is { } gaussdbDbType)
                    row["ProviderDbType"] = (int)gaussdbDbType;
            }
        }
        finally
    ...
    static async Task<DataTable> ParseResults(bool async, GaussDBCommand command, DataTable dataTable, Action<GaussDBDataReader, DataRow> populateRow, CancellationToken cancellationToken)
    {
    ...
    static object GetFieldValueOrDBNull<T>(GaussDBDataReader reader, int ordinal)
