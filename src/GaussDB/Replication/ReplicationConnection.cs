...
public abstract class ReplicationConnection : IAsyncDisposable
{
    ...
    readonly GaussDBConnection _gaussdbConnection;
    ...
    {
        _gaussdbConnection = new GaussDBConnection();
    ...
    /// <see cref="GaussDBConnectionStringBuilder.Pooling"/>, <see cref="GaussDBConnectionStringBuilder.Enlist"/>,
    /// <see cref="GaussDBConnectionStringBuilder.Multiplexing" /> and <see cref="GaussDBConnectionStringBuilder.KeepAlive"/>
    ...
    public string ConnectionString {
        ...
        set
        {
            ...
            var cs = new GaussDBConnectionStringBuilder(value)
            {
                Pooling = false,
                Enlist = false,
                Multiplexing = false,
            ...
            _gaussdbConnection.ConnectionString = cs.ToString();
        }
    }
    ...
    /// </summary>
    public GaussDBLogSequenceNumber LastReceivedLsn
    {
        get => (GaussDBLogSequenceNumber)unchecked((ulong)Interlocked.Read(ref _lastReceivedLsn));
    ...
    /// </summary>
    public GaussDBLogSequenceNumber LastFlushedLsn
    {
        get => (GaussDBLogSequenceNumber)unchecked((ulong)Interlocked.Read(ref _lastFlushedLsn));
    ...
    /// </summary>
    public GaussDBLogSequenceNumber LastAppliedLsn
    {
        get => (GaussDBLogSequenceNumber)unchecked((ulong)Interlocked.Read(ref _lastAppliedLsn));
    ...
    /// </remarks>
    /// </summary>
    public Version PostgreSqlVersion => _gaussdbConnection.PostgreSqlVersion;
    ...
    /// </remarks>
    /// </summary>
    public string ServerVersion => _gaussdbConnection.ServerVersion;

    internal GaussDBConnector Connector
        => _gaussdbConnection.Connector ??
    ...
    {
        get => _commandTimeout ?? (_gaussdbConnection.CommandTimeout > 0
            ? TimeSpan.FromSeconds(_gaussdbConnection.CommandTimeout)
    ...
    /// </summary>
    public Encoding Encoding => _gaussdbConnection.Connector?.TextEncoding ?? throw new InvalidOperationException($"The {nameof(Encoding)} property can only be used when there is an active connection");

    /// <summary>
    /// Process id of backend server.
    /// This can only be called when there is an active connection.
    /// </summary>
    public int ProcessID => _gaussdbConnection.Connector?.BackendProcessId ?? throw new InvalidOperationException($"The {nameof(ProcessID)} property can only be used when there is an active connection");
    ...
    public async Task Open(CancellationToken cancellationToken = default)
    {
        ...
        await _gaussdbConnection.OpenAsync(cancellationToken).ConfigureAwait(false);

        // PG versions before 10 ignore cancellations during replication
        _pgCancellationSupported = _gaussdbConnection.PostgreSqlVersion.IsGreaterOrEqual(10);

        SetTimeouts(CommandTimeout, CommandTimeout);

        ReplicationLogger = _gaussdbConnection.Connector!.LoggingConfiguration.ReplicationLogger;
    }
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        if (_gaussdbConnection.Connector?.State == ConnectorState.Replication)
        {
        ...
        try
        {
            await _gaussdbConnection.Close(async: true).ConfigureAwait(false);
        }
        catch
        {
            // Dispose
    ...
    public async Task<ReplicationSystemIdentification> IdentifySystem(CancellationToken cancellationToken = default)
    {
        ...
        return new ReplicationSystemIdentification(
            (string)row[0], (uint)row[1], GaussDBLogSequenceNumber.Parse((string)row[2]), (string)row[3]);
    }
    /// <summary>
    ...
    internal async Task<ReplicationSlotOptions> CreateReplicationSlot(string command, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return new ReplicationSlotOptions(slotName, GaussDBLogSequenceNumber.Parse(consistentPoint), snapshotName);
        }
        ...
        switch (slotType)
        {
            case "physical":
                ...
                return new PhysicalReplicationSlot(
                    ...
                    restartLsn == null ? null : GaussDBLogSequenceNumber.Parse(restartLsn),
                    restartTli);
            case null:
                return null;
            default:
                throw new NotSupportedException(
                    $"The replication slot type '{slotType}' is currently not supported by GaussDB. Please file an issue.");
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var connector = _gaussdbConnection.Connector!;
        ...
        GaussDBReadBuffer.ColumnStream? columnStream = null;
        ...
        try
        {
            ...
            columnStream = new GaussDBReadBuffer.ColumnStream(connector);
            ...
            while (true)
            {
                ...
                case 'w': // XLogData
                {
                    ...
                    _cachedXLogDataMessage.Populate(new GaussDBLogSequenceNumber(startLsn), new GaussDBLogSequenceNumber(endLsn),
                ...
                case 'k': // Primary keepalive message
                {
                    ...
                    {
                        var endLsn = new GaussDBLogSequenceNumber(end);
                ...
                default:
                    throw Connector.Break(new GaussDBException($"Unknown replication message code '{code}'"));
                }
            }
        }
        finally
    ...
    public void SetReplicationStatus(GaussDBLogSequenceNumber lastAppliedAndFlushedLsn)
    {
    ...
    public async Task SendStatusUpdate(CancellationToken cancellationToken = default)
    {
        ...
        // However, we generally aren't safe for this in GaussDB, leaving as-is for now.
        if (Connector.State != ConnectorState.Replication)
    ...
    async Task SendFeedback(bool waitOnSemaphore = false, bool requestReply = false, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (ReplicationLogger.IsEnabled(LogLevel.Trace))
            {
                LogMessages.SentReplicationFeedbackMessage(
                    ReplicationLogger,
                    new GaussDBLogSequenceNumber(unchecked((ulong)lastReceivedLsn)),
                    new GaussDBLogSequenceNumber(unchecked((ulong)lastFlushedLsn)),
                    new GaussDBLogSequenceNumber(unchecked((ulong)lastAppliedLsn)),
                    timestamp,
                    Connector.Id);
            }
        }
        ...
        {
            LogMessages.ReplicationFeedbackMessageSendingFailed(ReplicationLogger, _gaussdbConnection?.Connector?.Id, e);
        }
        finally
        {
    ...
    async Task<object[]> ReadSingleRow(string command, CancellationToken cancellationToken = default)
    {
        ...
        for (var i = 0; i < results.Length; i++)
        {
            ...
            case "bytea":
                ...
                {
                    throw Connector.Break(
                        new GaussDBException($"Could not parse data as bytea in field {field.Name}", e));
                }

                continue;
            default:

                throw Connector.Break(new GaussDBException(
