...
/// </summary>
/// <remarks>
/// Note that a connection may be closed before its TransactionScope completes. In this case we close the GaussDBConnection
...
sealed class VolatileResourceManager : ISinglePhaseNotification
{
    ...
    GaussDBTransaction _localTx = null!;
    ...
    internal VolatileResourceManager(GaussDBConnection connection, Transaction transaction)
    {
        _connector = connection.Connector!;
        _dataSource = connection.GaussDBDataSource;
    ...
    void RollbackLocal()
    {
        ...
        try
        {
            ...
            while (true)
            {
                ...
                catch (GaussDBOperationInProgressException)
                {
    ...
    void RollbackTwoPhase()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                // requests a commit at that exact time.
                // To avoid this, we open a new connection for performing the 2nd phase.
                using var conn2 = (GaussDBConnection)((ICloneable)_connector.Connection).Clone();
                conn2.Open();
...
#pragma warning disable CS8625
    void Dispose()
    {
        ...
        else
        {
            ...
            // Note that we remove it from the GaussDBDataSource bound to connection and not to connector
            // because of GaussDBMultiHostDataSource which has its own list to which connection adds connectors.
