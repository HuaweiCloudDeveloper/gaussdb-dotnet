...
public sealed class GaussDBDataReader : DbDataReader, IDbColumnSchemaGenerator
#pragma warning restore CA1010
{
    static readonly Task<bool> TrueTask = Task.FromResult(true);
    static readonly Task<bool> FalseTask = Task.FromResult(false);

    internal GaussDBCommand Command { get; private set; } = default!;
    internal GaussDBConnector Connector { get; }
    GaussDBConnection? _connection;
    ...
    internal GaussDBNestedDataReader? CachedFreeNestedDataReader;

    long _startTimestamp;
    readonly ILogger _commandLogger;

    internal GaussDBDataReader(GaussDBConnector connector)
    {
        Connector = connector;
    ...
    internal void Init(
        ...
        List<GaussDBBatchCommand> statements,
    ...
    ValueTask<IBackendMessage> ReadMessage(bool async)
    {
        ...
        static async ValueTask<IBackendMessage> ReadMessageSequential(GaussDBConnector connector, bool async)
        {
                ...
                else
                {
                    ...
                    case BackendMessageCode.CopyInResponse:
                        throw Connector.Break(new NotSupportedException(
                            "COPY isn't supported in regular command execution - see https://www.gaussdb.org/doc/copy.html for documentation on COPY with GaussDB. " +
    ...
    void PopulateOutputParameters(GaussDBParameterCollection parameters)
    {
        ...
        var taken = new List<GaussDBParameter>();
        ...
        foreach (var p in (IEnumerable<GaussDBParameter>)parameters)
        {
    ...
    /// a statement-by-statement basis, unlike <see cref="GaussDBDataReader.RecordsAffected"/>
    /// which exposes an aggregation across all statements.
    /// </remarks>
    [Obsolete("Use the new DbBatch API")]
    public IReadOnlyList<GaussDBBatchCommand> Statements
    {
        get
        {
            ThrowIfClosedOrDisposed();
    ...
    /// <summary>
    /// Releases the resources used by the <see cref="GaussDBDataReader"/>.
    /// </summary>
    protected override void Dispose(bool disposing)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (!(ex is PostgresException ||
                  ex is GaussDBException { InnerException: AggregateException aggregateException } &&
    ...
    /// <summary>
    /// Closes the <see cref="GaussDBDataReader"/> reader, allowing a new command to be executed.
    /// </summary>
    ...
    internal async Task Close(bool async, bool connectionClosing, bool isDisposing)
    {
        ...
        case ConnectorState.Connecting:
            if (State != ReaderState.Consumed)
            {
                ...
                catch (Exception ex) when (ex is OperationCanceledException or GaussDBException { InnerException: TimeoutException })
                {
    ...
    internal async Task Cleanup(bool async, bool connectionClosing = false, bool isDisposing = false)
    {
        ...
        GaussDBEventSource.Log.CommandStop();
    ...
    /// The column type must be a record or a to GaussDB known composite type, or an array thereof.
    /// Currently only supported in non-sequential mode.
    /// </summary>
    /// <param name="ordinal">The zero-based column ordinal.</param>
    /// <returns>A data reader.</returns>
    public new GaussDBNestedDataReader GetData(int ordinal)
    {
        ThrowIfNotInResult();
        ...
        {
            reader = new GaussDBNestedDataReader(this, null, 1, compositeType);
        }
        if (isArray)
            reader.InitArray();
        else
    ...
    [UnconditionalSuppressMessage("ILLink", "IL2093",
        Justification = "Members are only dynamically accessed by GaussDB via GetFieldType by GetSchema, and only in certain cases. " +
    ...
    /// </summary>
    /// <returns></returns>
    public ReadOnlyCollection<GaussDBDbColumn> GetColumnSchema()
    ...
    /// </summary>
    /// <returns></returns>
    public new Task<ReadOnlyCollection<GaussDBDbColumn>> GetColumnSchemaAsync(CancellationToken cancellationToken = default)
        => GetColumnSchema(async: true, cancellationToken);

    Task<ReadOnlyCollection<GaussDBDbColumn>> GetColumnSchema(bool async, CancellationToken cancellationToken = default)
        => RowDescription == null || ColumnCount == 0
            ? Task.FromResult(new List<GaussDBDbColumn>().AsReadOnly())
    ...
    async Task<DataTable?> GetSchemaTable(bool async, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var column in await GetColumnSchema(async, cancellationToken).ConfigureAwait(false))
        {
            ...
            row["ProviderType"] = column.GaussDBDbType ?? GaussDBDbType.Unknown;
    ...
    static void ThrowInvalidState(ReaderState state)
    {
        ...
        case ReaderState.Closed:
            ...
            break;
        case ReaderState.Disposed:
            ThrowHelper.ThrowObjectDisposedException(nameof(GaussDBDataReader));
            break;
        default:
            ThrowHelper.ThrowInvalidOperationException("No resultset is currently being traversed");
            break;
    ...
    internal void UnbindIfNecessary()
    {
        ...
        if (State != ReaderState.Disposed)
        {
            Connector.DataReader = Connector.UnboundDataReader is { State: ReaderState.Disposed } previousReader
                ? previousReader
                : new GaussDBDataReader(Connector);
